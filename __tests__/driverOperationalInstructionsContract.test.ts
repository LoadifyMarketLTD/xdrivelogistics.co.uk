...
describe('Driver operational message contract', () => {
  const instructionRoute = fs.readFileSync(path.join(process.cwd(), 'app/api/workspace/jobs/[jobId]/instructions/route.ts'), 'utf8');
  const instructionPanel = fs.readFileSync(path.join(process.cwd(), 'app/components/workspace/DriverInstructionPanel.tsx'), 'utf8');
  const customerJobPage = fs.readFileSync(path.join(process.cwd(), 'app/customer/jobs/[id]/page.tsx'), 'utf8');
  const brokerJobsPage = fs.readFileSync(path.join(process.cwd(), 'app/broker/jobs/page.tsx'), 'utf8');
  const driverDetailRoute = fs.readFileSync(path.join(process.cwd(), 'app/api/driver/mobile/jobs/[id]/route.ts'), 'utf8');
  const driverResourcesRoute = fs.readFileSync(path.join(process.cwd(), 'app/api/driver/mobile/resources/route.ts'), 'utf8');
  ...
  test('queues permanent history and notifies the assigned Driver inbox when available', () => {
  ...
  test('reconciles the Driver inbox backend with the current mobile resources contract', () => {
