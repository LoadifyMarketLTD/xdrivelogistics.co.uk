...
BEGIN
  ...
  v_application_status := COALESCE(NULLIF(trim(v_application.status::text), ''), 'unknown');
  ...
  IF v_existing_id IS NULL THEN
    INSERT INTO public.platform_document_requests (
      ...
      jsonb_build_object('channel_primary','email','onboarding_status',v_application_status)
  ...
  ELSE
    ...
    SET requested_documents = v_missing,
        ...
        updated_at = now(),
        metadata = COALESCE(metadata,'{}'::jsonb) || jsonb_build_object('onboarding_status',v_application_status)
  ...
  INSERT INTO public.owner_audit_log (
    ...
    v_application_status, v_application_status,
