        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {GLOBAL_SETTINGS.map((group) => (
            <div key={group.group} style={{ backgroundColor: THEME.cardBg, border: `1px solid ${THEME.cardBorder}`, borderRadius: '12px', overflow: 'hidden' }}>
              <div style={{ padding: '0.75rem 1rem', borderBottom: `1px solid ${THEME.cardBorder}`, backgroundColor: '#1A1F2B' }}>
              ...
              <div style={{ padding: '0.25rem 0' }}>
                {group.settings.map((setting) => (
                  <div key={setting.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.6rem 1rem', borderBottom: `1px solid rgba(29, 87, 216, 0.4)` }}>
                    <div>
                      <div style={{ color: THEME.text, fontSize: '0.82rem', fontWeight: 500 }}>{setting.label}</div>
                      <div style={{ color: '#0B2F6B', fontSize: '0.68rem', fontFamily: 'monospace', marginTop: '0.1rem' }}>{setting.key}</div>
                    </div>
                    <div style={{ backgroundColor: '#1A1F2B', border: `1px solid ${THEME.cardBorder}`, borderRadius: '6px', padding: '0.3rem 0.75rem', color: THEME.text, fontSize: '0.82rem', fontFamily: setting.type === 'text' ? 'inherit' : 'monospace', minWidth: '120px', textAlign: 'right' }}>
                      {setting.value}
                    </div>
                  </div>
                ))}
