...
export async function GET(request: NextRequest) {
  ...
  const missingDocuments = ((rows ?? []) as MissingDocumentRow[])
    .map(documentName)
    .filter(Boolean);

  // Normal runtime may close a durable request once the canonical requirement set is complete.
  // Deploy Preview is strictly read-only, so even housekeeping must fail closed there.
  if (missingDocuments.length === 0 && !isDeployPreviewReadOnly()) {
    try {
      ...
      // Best-effort housekeeping must never block the read-only checklist response.
    }
  }
