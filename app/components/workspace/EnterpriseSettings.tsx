...
export function SettingsLayout({ children, title = 'Settings', description = 'Manage the settings available to your current role and company workspace.', actions, status, role: forcedRole }: { children: ReactNode; title?: string; description?: string; actions?: ReactNode; status?: ReactNode; role?: WorkspaceRole }) {
  const { user } = useAuth(); const role = forcedRole ?? resolveWorkspaceRole(user); const groups = useMemo(() => getEnterpriseSettingsGroups(role), [role]);
...
export function EnterpriseSettingsBoundary({ children, role: forcedRole }: { children: ReactNode; role?: WorkspaceRole }) { const pathname = usePathname(); const { user } = useAuth(); const role = forcedRole ?? resolveWorkspaceRole(user); if (!isEnterpriseSettingsRoute(pathname, role)) return <>{children}</>; return <SettingsLayout role={role}>{children}</SettingsLayout>; }
