...
alter table public.invoices
  ...
  add column if not exists issuer_address_snapshot text,
  add column if not exists issuer_company_number_snapshot text,
  add column if not exists issuer_xd_id_snapshot text,
  add column if not exists issuer_email_snapshot text,
  add column if not exists issuer_phone_snapshot text,
  add column if not exists customer_company_number_snapshot text,
  add column if not exists customer_xd_id_snapshot text,
  ...
  add column if not exists bank_sort_code_snapshot text,
...
begin
  ...
  if new.vehicle_id is not null then
    select nullif(btrim(coalesce(v.reg_plate, '')), '')
    into v_vehicle_registration
  ...
  set
    vehicle_type = coalesce(
      nullif(btrim(coalesce(new.requested_vehicle_label, '')), ''),
      nullif(btrim(coalesce(new.vehicle_type, '')), ''),
      nullif(btrim(coalesce(new.requested_vehicle_type, '')), ''),
      i.vehicle_type
    ),
    vehicle_registration = coalesce(v_vehicle_registration, i.vehicle_registration),
    load_id = coalesce(
      nullif(btrim(coalesce(new.load_ref, '')), ''),
      nullif(btrim(coalesce(new.load_id, '')), ''),
      nullif(btrim(coalesce(new.load_reference, '')), ''),
      'XDL-' || upper(substr(new.id::text, 1, 8)),
      i.load_id
    ),
    customer_ref = coalesce(
      nullif(btrim(coalesce(new.customer_reference, '')), ''),
      nullif(btrim(coalesce(new.customer_ref, '')), ''),
      nullif(btrim(coalesce(new.cust_ref, '')), ''),
      nullif(btrim(coalesce(new.your_ref, '')), ''),
      nullif(btrim(coalesce(new.booking_reference, '')), ''),
      i.customer_ref
    ),
    ...
    delivery_contact_name = coalesce(nullif(btrim(coalesce(new.delivery_contact_name, '')), ''), i.delivery_contact_name),
    delivery_contact_phone = coalesce(nullif(btrim(coalesce(new.delivery_contact_phone, '')), ''), i.delivery_contact_phone),
    ...
    collection_notes = coalesce(nullif(btrim(coalesce(new.collection_notes, '')), ''), i.collection_notes),
    ...
    pod_required = coalesce(new.pod_required, i.pod_required),
    pod_generated = coalesce(new.pod_generated, true),
    pod_generated_at = coalesce(new.pod_generated_at, v_pod.updated_at, now()),
    pod_photos = coalesce(to_jsonb(v_pod.photo_urls), new.delivery_photos, i.pod_photos),
    signature = coalesce(
      case
        when new.delivery_signature_data is not null and jsonb_typeof(new.delivery_signature_data) = 'string'
          then new.delivery_signature_data #>> '{}'
        else null
      end,
      i.signature
    ),
    recipient_name = coalesce(nullif(btrim(coalesce(v_pod.received_by, '')), ''), i.recipient_name),
    delivery_state = coalesce(nullif(btrim(coalesce(v_pod.delivery_status, '')), ''), nullif(btrim(coalesce(new.delivery_status, '')), ''), i.delivery_state),
    ...
    cargo_summary = coalesce(
      nullif(concat_ws(' · ',
        case when new.pallets is not null then new.pallets::text || case when new.pallets = 1 then ' pallet' else ' pallets' end end,
        case when new.weight_kg is not null then trim(to_char(new.weight_kg, 'FM999999990.##')) || ' kg' end
      ), ''),
      i.cargo_summary
    ),
    issuer_name_snapshot = coalesce(
      (select coalesce(nullif(btrim(c.legal_name), ''), nullif(btrim(c.trading_name), ''), nullif(btrim(c.name), '')) from public.companies c where c.id = i.supplier_company_id),
      i.issuer_name_snapshot
    ),
    issuer_address_snapshot = coalesce(
      (select nullif(concat_ws(', ',
        nullif(btrim(c.address_line1), ''),
        nullif(btrim(c.address_line2), ''),
        nullif(btrim(c.city), ''),
        nullif(btrim(c.postcode), ''),
        nullif(btrim(c.country), '')
      ), '') from public.companies c where c.id = i.supplier_company_id),
      i.issuer_address_snapshot
    ),
...
drop trigger if exists zz_trg_enrich_invoice_on_job_completion on public.jobs;
create trigger zz_trg_enrich_invoice_on_job_completion
after update of status, current_status, delivered_at, completed_at on public.jobs
for each row
execute function public.fn_enrich_invoice_on_job_completion();
