...
export default function AvailableLoadsPage() {
  ...
  const captureFilters = (): SavedLoadFilters => ({
    vehicleFilter, pickupFilter, deliveryFilter, cargoFilter, weightMinFilter, dateFromFilter, dateToFilter,
    memberFilter, regionFilter, postedWithinFilter, jobTimingFilter, sortBy,
  });
                  ...
                  return (
                    <article key={load.id} className="driver-load-row" data-state={quoted ? 'quoted' : 'open'}>
                      <div className="driver-load-row__top">
                        ...
                        <div className="driver-load-cell"><span className="driver-cell-label">Load</span><strong className="driver-cell-primary">{vehicleLabel}</strong><span className="driver-cell-secondary">{cargoLabel}{load.weight_kg ? ` · ${load.weight_kg} kg` : ''}{load.pallets ? ` · ${load.pallets} pallet${load.pallets === 1 ? '' : 's'}` : ''}</span></div>
                        <div className="driver-load-cell"><span className="driver-cell-label">Commercial</span><strong className="driver-cell-primary">{hasProposedPrice ? money(load.budget_amount, load.currency) : 'Quote required'}</strong><span className="driver-cell-secondary">{load.member.name}{load.member.memberId ? ` · ${load.member.memberId}` : ''} · posted {fmtDate(load.exchange_posted_at)}</span></div>
                      </div>
                      ...
                      <div className="driver-load-row__meta">
                        ...
                        <div className="driver-row-actions">
                          {!quoted && <ActionButton tone="success" onClick={() => { setExpandAll(false); setExpandedLoadId(load.id); setBidLoadId(load.id); setBidAmount(hasProposedPrice && load.budget_amount != null ? String(load.budget_amount) : ''); setBidMessage(''); }}>Quote Now</ActionButton>}
                      ...
                      {expanded && (
                        <div className="driver-row-details">
                          <div className="driver-detail-grid">
                            ...
                            <div className="driver-detail-item"><span>Posting member</span><strong>{load.member.name}</strong><small>{[load.member.memberType, load.member.memberId].filter(Boolean).join(' · ') || 'Member identity available'}</small></div>
