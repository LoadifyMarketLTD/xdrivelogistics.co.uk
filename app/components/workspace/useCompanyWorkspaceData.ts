...
export type WorkspaceLocation={id:string;driver_id:string;job_id?:string|null;recorded_at?:string|null;updated_at?:string|null;location?:unknown};
...
export function useCompanyWorkspaceData(){
 const {user}=useAuth();const [companyId,setCompanyId]=useState<string|null>(user?.companyId??null);const [loading,setLoading]=useState(true);const [error,setError]=useState('');
 const [jobs,setJobs]=useState<WorkspaceJob[]>([]);const [bids,setBids]=useState<WorkspaceBid[]>([]);const [invoices,setInvoices]=useState<WorkspaceInvoice[]>([]);const [drivers,setDrivers]=useState<WorkspaceDriver[]>([]);const [vehicles,setVehicles]=useState<WorkspaceVehicle[]>([]);const [driverDocuments,setDriverDocuments]=useState<WorkspaceDocument[]>([]);const [vehicleDocuments,setVehicleDocuments]=useState<WorkspaceDocument[]>([]);const [locations,setLocations]=useState<WorkspaceLocation[]>([]);
 ...
 const refresh=useCallback(async()=>{
  ...
  const jobRows=rows<WorkspaceJob>(jobsRes as QueryResult<WorkspaceJob>,errors);const jobIds=jobRows.map(j=>j.id);
  const [ownBidsRes,receivedBidsRes,invoicesRes,driversRes,vehiclesRes]=await Promise.all([
  ...
  const driverRows=rows<WorkspaceDriver>(driversRes as QueryResult<WorkspaceDriver>,errors);const vehicleRows=rows<WorkspaceVehicle>(vehiclesRes as QueryResult<WorkspaceVehicle>,errors);const driverIds=driverRows.map(d=>d.id);
  const [driverDocsRes,vehicleDocsRes,locationsRes]=await Promise.all([
   driverIds.length?supabase.from('driver_documents').select('id, driver_id, doc_type, status, expiry_date').in('driver_id',driverIds).order('expiry_date',{ascending:true}).limit(1000):Promise.resolve({data:[] as WorkspaceDocument[],error:null}),
   ...
   driverIds.length?supabase.from('driver_locations').select('id, driver_id, job_id, recorded_at, location').in('driver_id',driverIds).order('recorded_at',{ascending:false}).limit(500):Promise.resolve({data:[] as WorkspaceLocation[],error:null}),
  ]);
  setJobs(jobRows);setBids(unique([...rows<WorkspaceBid>(ownBidsRes as QueryResult<WorkspaceBid>,errors),...rows<WorkspaceBid>(receivedBidsRes as QueryResult<WorkspaceBid>,errors)]).sort((a,b)=>+new Date(b.created_at)-+new Date(a.created_at)));setInvoices(rows<WorkspaceInvoice>(invoicesRes as QueryResult<WorkspaceInvoice>,errors));setDrivers(driverRows);setVehicles(vehicleRows);setDriverDocuments(rows<WorkspaceDocument>(driverDocsRes as QueryResult<WorkspaceDocument>,errors));setVehicleDocuments(rows<WorkspaceDocument>(vehicleDocsRes as QueryResult<WorkspaceDocument>,errors));setLocations(rows<WorkspaceLocation>(locationsRes as QueryResult<WorkspaceLocation>,errors));setError(errors.length?`Some workspace data could not be loaded: ${errors[0]}`:'');setLoading(false);
 },[companyId]);
