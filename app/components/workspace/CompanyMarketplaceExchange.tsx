...
export default function CompanyMarketplaceExchange() {
  ...
  const mapLoads = useMemo(() => loads.map((load) => ({ id: load.id, pickupLabel: routeLabel(load.pickup_location, load.pickup_postcode), pickupPostcode: load.pickup_postcode, deliveryLabel: routeLabel(load.delivery_location, load.delivery_postcode), vehicleLabel: vehicleLabel(load), posterName: load.posterName, pickupAt: load.pickup_datetime, postedAt: load.exchange_posted_at })), [loads]);
  ...
  return (
    <PageFrame>
      ...
      {tab === 'loads' && <>
        <Panel title="Search Loads" description="Radius search uses UK postcode/outcode geocoding where available; otherwise the search falls back to text matching.">
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(150px,1.3fr) 80px minmax(150px,1.3fr) 80px repeat(2,minmax(130px,1fr))', gap: 8, alignItems: 'end' }}>
            <label><span style={labelStyle}>FROM</span><input value={filters.from} onChange={(e) => setFilter('from', e.target.value)} placeholder="Postcode / area" style={{ ...fieldStyle, width: '100%' }} /></label><label><span style={labelStyle}>Radius</span><select value={filters.fromRadius} onChange={(e) => setFilter('fromRadius', e.target.value)} style={{ ...fieldStyle, width: '100%' }}>{radiusOptions.map((value) => <option key={value} value={value}>{value} mi</option>)}</select></label>
            <label><span style={labelStyle}>TO</span><input value={filters.to} onChange={(e) => setFilter('to', e.target.value)} placeholder="Postcode / area" style={{ ...fieldStyle, width: '100%' }} /></label><label><span style={labelStyle}>Radius</span><select value={filters.toRadius} onChange={(e) => setFilter('toRadius', e.target.value)} style={{ ...fieldStyle, width: '100%' }}>{radiusOptions.map((value) => <option key={value} value={value}>{value} mi</option>)}</select></label>
            <label><span style={labelStyle}>Minimum vehicle</span><select value={filters.minVehicle} onChange={(e) => setFilter('minVehicle', e.target.value)} style={{ ...fieldStyle, width: '100%' }}><option value="">No minimum</option>{VEHICLE_SIZE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label><span style={labelStyle}>Maximum vehicle</span><select value={filters.maxVehicle} onChange={(e) => setFilter('maxVehicle', e.target.value)} style={{ ...fieldStyle, width: '100%' }}><option value="">No maximum</option>{VEHICLE_SIZE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,minmax(120px,1fr))', gap: 8, alignItems: 'end', marginTop: 8 }}>
            <label><span style={labelStyle}>Exact / specialist vehicle</span><select value={filters.vehicle} onChange={(e) => setFilter('vehicle', e.target.value)} style={{ ...fieldStyle, width: '100%' }}>{VEHICLE_OPTIONS.map(([value, label]) => <option key={value || 'any'} value={value}>{label}</option>)}</select></label><label><span style={labelStyle}>Body / equipment</span><input value={filters.body} onChange={(e) => setFilter('body', e.target.value)} placeholder="Curtain, tail lift…" style={{ ...fieldStyle, width: '100%' }} /></label><label><span style={labelStyle}>Freight</span><input value={filters.freight} onChange={(e) => setFilter('freight', e.target.value)} placeholder="Pallets, parcels…" style={{ ...fieldStyle, width: '100%' }} /></label><label><span style={labelStyle}>Member Name / ID</span><input value={filters.member} onChange={(e) => setFilter('member', e.target.value)} placeholder="Company / member ID" style={{ ...fieldStyle, width: '100%' }} /></label><label><span style={labelStyle}>Job timing</span><select value={filters.description} onChange={(e) => setFilter('description', e.target.value)} style={{ ...fieldStyle, width: '100%' }}>{DESCRIPTION_OPTIONS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label><span style={labelStyle}>Posted within</span><select value={filters.postedWithinHours} onChange={(e) => setFilter('postedWithinHours', e.target.value)} style={{ ...fieldStyle, width: '100%' }}><option value="">Any time</option><option value="1">1 hour</option><option value="3">3 hours</option><option value="6">6 hours</option><option value="12">12 hours</option><option value="24">24 hours</option><option value="48">48 hours</option></select></label>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,minmax(120px,1fr))', gap: 8, alignItems: 'end', marginTop: 8 }}>
            <label><span style={labelStyle}>Pickup from</span><input type="date" value={filters.dateFrom} onChange={(e) => setFilter('dateFrom', e.target.value)} style={{ ...fieldStyle, width: '100%' }} /></label><label><span style={labelStyle}>Pickup to</span><input type="date" value={filters.dateTo} onChange={(e) => setFilter('dateTo', e.target.value)} style={{ ...fieldStyle, width: '100%' }} /></label><label><span style={labelStyle}>Min budget</span><input type="number" min="0" value={filters.minBudget} onChange={(e) => setFilter('minBudget', e.target.value)} placeholder="£0" style={{ ...fieldStyle, width: '100%' }} /></label><label><span style={labelStyle}>Max budget</span><input type="number" min="0" value={filters.maxBudget} onChange={(e) => setFilter('maxBudget', e.target.value)} placeholder="No maximum" style={{ ...fieldStyle, width: '100%' }} /></label><label><span style={labelStyle}>Results</span><select value={filters.pageSize} onChange={(e) => setFilter('pageSize', e.target.value)} style={{ ...fieldStyle, width: '100%' }}><option value="10">10 / page</option><option value="25">25 / page</option><option value="50">50 / page</option></select></label>
            <div><span style={labelStyle}>Recent searches</span><select defaultValue="" onChange={(e) => { const found = recentSearches.find((item) => item.id === e.target.value); if (found) { setFilters({ ...DEFAULT_FILTERS, ...found.filters }); setPage(1); setNotice(`Recent search loaded: ${found.label}. Press Search to refresh results.`); } e.currentTarget.value = ''; }} style={{ ...fieldStyle, width: '100%' }}><option value="">Select recent search…</option>{recentSearches.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></div>
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, alignItems: 'center', marginTop: 8 }}><ActionButton tone="secondary" onClick={applyDefault}>Load Default</ActionButton><ActionButton tone="secondary" onClick={clearFilters}>Clear</ActionButton><ActionButton tone="success" onClick={() => void loadLoads(1, true)} disabled={loading}>{loading ? 'Searching…' : 'Search'}</ActionButton></div>
        </Panel>

        <div style={{ minHeight: 36, display: 'flex', alignItems: 'center', gap: 4, margin: '8px 0', borderBottom: '1px solid #dbe2ea' }}>{LOAD_TYPES.map(([value, label]) => <button key={value} type="button" onClick={() => { setFilter('loadType', value); setTimeout(() => void loadLoads(1, true), 0); }} style={{ height: 28, border: 0, borderBottom: filters.loadType === value ? '2px solid #1d57d8' : '2px solid transparent', background: 'transparent', color: filters.loadType === value ? '#1d57d8' : '#64748b', fontSize: '11px', fontWeight: filters.loadType === value ? 800 : 650, padding: '0 10px', cursor: 'pointer' }}>{label}</button>)}<span style={{ marginLeft: 'auto', color: '#64748b', fontSize: '11px' }}>{total} result{total === 1 ? '' : 's'}</span>{viewMode === 'list' && <OperationalExpandAllControl expanded={allVisibleExpanded} disabled={!loads.length} onToggle={toggleExpandAll} noun="loads" />}<ActionButton tone="secondary" onClick={() => setViewMode('list')}>List View</ActionButton><ActionButton tone="secondary" onClick={() => setViewMode('map')}>Map View</ActionButton></div>

        {viewMode === 'map' ? <MarketplaceLoadMap loads={mapLoads} onQuote={(loadId) => { const target = loads.find((load) => load.id === loadId); if (target) openQuote(target); }} onDetails={(loadId) => { setViewMode('list'); setExpanded((current) => new Set(current).add(loadId)); }} /> : <Panel title="Available Loads" description="Exchange and direct-invite work visible to your active company workspace." flush><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1020, fontSize: '12px' }}><thead><tr style={{ background: '#f8fafc', color: '#475569', textAlign: 'left' }}>{['Load', 'Route', 'Pickup', 'Vehicle', 'Freight', 'Member', 'Price', 'Type', 'Quote', 'Action'].map((heading) => <th key={heading} style={{ height: 40, padding: '0 8px', borderBottom: '1px solid #dbe2ea', position: 'sticky', top: 0, background: '#f8fafc', whiteSpace: 'nowrap', fontSize: 11 }}>{heading}</th>)}</tr></thead><tbody>
          {loads.map((load) => { const isExpanded = expanded.has(load.id); const amount = Number(load.budget_amount); const hasBudget = Number.isFinite(amount) && amount > 0; return [<tr key={load.id} style={{ minHeight: 42, borderBottom: '1px solid #edf2f7', background: load.exchange_visibility === 'direct' ? '#fffaf0' : '#fff' }}><td style={{ padding: '8px', fontWeight: 800 }}>{load.id.slice(0, 8).toUpperCase()}{load.exchange_visibility === 'direct' && <div style={{ marginTop: 3 }}><StatusBadge value="Direct invite" tone="orange" /></div>}</td><td style={{ padding: '8px' }}><strong>{routeLabel(load.pickup_location, load.pickup_postcode)} → {routeLabel(load.delivery_location, load.delivery_postcode)}</strong>{load.journeyDistanceMiles != null && <div style={{ color: '#64748b', marginTop: 2 }}>{load.journeyDistanceMiles.toFixed(1)} mi route</div>}</td><td style={{ padding: '8px', whiteSpace: 'nowrap' }}>{when(load.pickup_datetime)}{load.pickup_time_slot && <div style={{ color: '#64748b' }}>{load.pickup_time_slot}</div>}</td><td style={{ padding: '8px', textTransform: 'capitalize' }}>{vehicleLabel(load)}</td><td style={{ padding: '8px' }}>{load.requested_cargo_label || load.cargo_type?.replace(/_/g, ' ') || '—'}{load.weight_kg != null && <div style={{ color: '#64748b' }}>{Number(load.weight_kg).toLocaleString()} kg</div>}</td><td style={{ padding: '8px' }}>{load.posterName}{load.posterMemberCode && <div style={{ color: '#64748b' }}>ID {load.posterMemberCode}</div>}</td><td style={{ padding: '8px', whiteSpace: 'nowrap' }}>{hasBudget ? <><strong>{money(amount, load.currency || 'GBP')}</strong><div style={{ color: '#64748b' }}>{load.is_fixed_price ? 'Fixed' : 'Budget'}</div></> : '—'}</td><td style={{ padding: '8px' }}>{loadTypeLabel(load.loadType)}<div style={{ color: '#64748b' }}>{descriptionLabel(load.jobDescription)}</div></td><td style={{ padding: '8px' }}>{load.myBid ? <><StatusBadge value={load.myBid.status} /><div style={{ marginTop: 3 }}>{money(bidAmount(load.myBid), load.myBid.currency || 'GBP')}</div></> : <span style={{ color: '#64748b' }}>Not quoted</span>}</td><td style={{ padding: '8px', whiteSpace: 'nowrap' }}><div style={{ display: 'flex', gap: 4 }}><ActionButton tone="secondary" onClick={() => setExpanded((current) => { const next = new Set(current); if (next.has(load.id)) next.delete(load.id); else next.add(load.id); return next; })}>{isExpanded ? 'Close' : 'Details'}</ActionButton>{!load.myBid || ['withdrawn', 'rejected', 'unsuccessful'].includes(load.myBid.status) ? <ActionButton tone="success" onClick={() => openQuote(load)}>Quote</ActionButton> : null}</div></td></tr>, isExpanded ? <tr key={`${load.id}-details`} style={{ background: '#f8fafc' }}><td colSpan={10} style={{ padding: '8px 10px', borderBottom: '1px solid #dbe2ea' }}><div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 12 }}><div><strong>Pickup</strong><div>{load.pickup_location || load.pickup_postcode || '—'}</div><div style={{ color: '#64748b' }}>{load.distanceFromSearchOriginMiles != null ? `${load.distanceFromSearchOriginMiles.toFixed(1)} mi from FROM search` : 'Radius distance unavailable'}</div></div><div><strong>Delivery</strong><div>{load.delivery_location || load.delivery_postcode || '—'}</div><div style={{ color: '#64748b' }}>{load.distanceToSearchDestinationMiles != null ? `${load.distanceToSearchDestinationMiles.toFixed(1)} mi from TO search` : 'Radius distance unavailable'}</div></div><div><strong>References</strong><div>Customer: {load.customer_reference || '—'}</div><div>Booking: {load.booking_reference || '—'}</div></div><div><strong>Requirements</strong><div>{load.special_requirements || 'None stated'}</div><div style={{ color: '#64748b' }}>{load.access_restrictions || 'No access restrictions stated'}</div></div></div><div style={{ marginTop: 8, display: 'flex', gap: 4 }}><ActionButton tone="secondary" onClick={() => window.open(`https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(load.pickup_postcode || load.pickup_location || '')}&destination=${encodeURIComponent(load.delivery_postcode || load.delivery_location || '')}`, '_blank', 'noopener,noreferrer')}>Open Route</ActionButton>{!load.myBid || ['withdrawn', 'rejected', 'unsuccessful'].includes(load.myBid.status) ? <ActionButton tone="success" onClick={() => openQuote(load)}>Submit Quote</ActionButton> : null}</div></td></tr> : null]; })}
          {!loading && loads.length === 0 && <tr><td colSpan={10} style={{ padding: 28, textAlign: 'center', color: '#64748b' }}>No live loads match this search.</td></tr>}
      ...
      {tab === 'bids' && <Panel title="My Quotes" description="Commercial responses submitted by your company to marketplace loads." flush><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900, fontSize: '12px' }}><thead><tr style={{ background: '#f8fafc', color: '#475569', textAlign: 'left' }}>{['Quote', 'Load / route', 'Member', 'Amount', 'Submitted', 'Status', 'Action'].map((heading) => <th key={heading} style={{ height: 40, padding: '0 8px', borderBottom: '1px solid #dbe2ea', fontSize: 11 }}>{heading}</th>)}</tr></thead><tbody>{bids.map((bid) => <tr key={bid.id} style={{ borderBottom: '1px solid #edf2f7' }}><td style={{ padding: '8px', fontWeight: 800 }}>{bid.id.slice(0, 8).toUpperCase()}</td><td style={{ padding: '8px' }}><strong>{routeLabel(bid.job?.pickup_location, bid.job?.pickup_postcode)} → {routeLabel(bid.job?.delivery_location, bid.job?.delivery_postcode)}</strong><div style={{ color: '#64748b' }}>Load {bid.job_id.slice(0, 8).toUpperCase()}</div></td><td style={{ padding: '8px' }}>{bid.job?.posterName || 'Marketplace member'}{bid.job?.posterMemberCode && <div style={{ color: '#64748b' }}>ID {bid.job.posterMemberCode}</div>}</td><td style={{ padding: '8px', fontWeight: 800 }}>{money(bidAmount(bid), bid.currency || 'GBP')}</td><td style={{ padding: '8px' }}>{when(bid.created_at)}</td><td style={{ padding: '8px' }}><StatusBadge value={bid.status} /></td><td style={{ padding: '8px' }}>{bid.status === 'submitted' ? <ActionButton tone="secondary" disabled={working} onClick={() => void withdrawQuote(bid.id)}>Withdraw</ActionButton> : '—'}</td></tr>)}{!loading && bids.length === 0 && <tr><td colSpan={7} style={{ padding: 28, textAlign: 'center', color: '#64748b' }}>No marketplace quotes have been submitted by this company.</td></tr>}</tbody></table></div></Panel>}
