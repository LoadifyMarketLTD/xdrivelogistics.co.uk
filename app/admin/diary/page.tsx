...
export default function DiaryPage() {
  ...
  const loadJobs = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('jobs')
      .select([
        'id','status','current_status','assigned_driver_id','assigned_company_id','awarded_carrier_company_id',
      ...
      <div style={{ display: 'flex', height: 'calc(100vh - 89px)', overflow: 'hidden', background: '#f5f7fa' }}>
        ...
        <main style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ padding: '0.75rem', flex: 1, overflowY: 'auto' }}>
            {loading ? (<div style={emptyCard}>Loading diary…</div>) : filteredJobs.length === 0 ? (<div style={emptyCard}><div style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>📋</div><div style={{ fontSize: '0.88rem' }}>No bookings in this category.</div></div>) : (
