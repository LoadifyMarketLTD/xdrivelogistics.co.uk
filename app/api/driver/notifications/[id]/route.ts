...
export async function DELETE(
  ...
  if (!isSupabaseAdminConfigured || !supabaseAdmin) {
    return json(503, { error: 'Notification removal is temporarily unavailable.' });
  }
  ...
  if (error) return json(500, { error: 'This notification could not be removed.' });
  if (!data) return json(404, { error: 'Notification not found.' });
  return json(200, { success: true, notificationId });
}
