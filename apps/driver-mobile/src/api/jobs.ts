...
async function uploadCollectionPhoto(
  ...
  if (!uri.includes('://')) {
    // Server storage paths are never accepted from the mobile status payload.
    // The evidence route is the only authority allowed to link collection proof.
  ...
  // Keep the id in the caller-owned payload. A transient online failure that is
  // subsequently enqueued must replay the same object name rather than create a
  // second private storage object.
...
export async function postJobStatus(
  ...
  // Loaded is evidence-gated. The collection image goes through the private,
  // assignment-gated evidence endpoint first; the status endpoint never trusts
  // a client-supplied storage path and reads jobs.collection_photo_url instead.
