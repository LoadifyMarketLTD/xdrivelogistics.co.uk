...
const MEMBERSHIP_SELECT = 'id, company_id, user_id, role_in_company, status, companies(id, name, company_type, status)';
...
describe('resolveAuthenticatedUser driver schema compatibility', () => {
  ...
  it('preserves commercial facts when full driver schema columns are available', async () => {
    mockState.expectations = [
      { table: 'profiles', select: PROFILE_SELECT, result: ok({ ...baseProfile, company_id: 'company-1' }) },
