-- Prevent canonical Marketplace delivery from being rolled back by invoice
-- snapshot integrity. The delivery trigger introduced in 20260818125500 must
-- supply the required invoice_number before the invoice BEFORE INSERT integrity
-- trigger executes. No finance roles, lifecycle states or visibility rules are
-- changed here.
...
BEGIN
  ...
  IF NULLIF(btrim(COALESCE(v_invoice_number, '')), '') IS NULL THEN
    RAISE EXCEPTION 'Marketplace invoice number could not be generated.' USING ERRCODE = '23514';
  END IF;
...
COMMENT ON FUNCTION public.fn_generate_invoice_on_job_completion() IS
  'Canonical Marketplace delivery invoice generator: creates at most one draft supplier invoice from the accepted commercial agreement, with a required company-scoped invoice number generated before snapshot validation.';
