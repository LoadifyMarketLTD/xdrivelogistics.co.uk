...
describe('MASTER CONTRACT FINAL v2 — integrated Super Admin contract', () => {
  ...
  it('locks Operations Control Centre, Jobs, Drivers and Fleet without forbidden one-column grid collapse', () => {
    ...
    expect(cockpit).toContain('No drivers currently have a valid live GPS position.');
    expect(cockpitApi).toContain("from('driver_availability_presence')");
    expect(cockpitApi).toContain("driver_availability_presence', 'driver_locations'");
    expect(cockpitApi).toContain("source: 'availability_presence'");
    expect(cockpitApi).toContain("location, lat, lng, heading, speed_mph");
