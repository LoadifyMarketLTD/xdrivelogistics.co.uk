...
-- Platform settings and feature flags affect control-plane behaviour globally.
-- Mutations are therefore executed and audited in one PostgreSQL transaction.
DO $preflight$
BEGIN
  IF to_regclass('public.platform_feature_flags') IS NULL THEN
    RAISE EXCEPTION 'public.platform_feature_flags must exist before applying Platform settings governance.'
      USING ERRCODE = '23514';
  END IF;

  IF to_regclass('public.platform_settings') IS NULL THEN
    RAISE EXCEPTION 'public.platform_settings must exist before applying Platform settings governance.'
      USING ERRCODE = '23514';
  END IF;

  IF to_regclass('public.owner_audit_log') IS NULL THEN
    RAISE EXCEPTION 'public.owner_audit_log must exist before applying Platform settings governance.'
      USING ERRCODE = '23514';
  END IF;

  IF to_regprocedure('public.assert_platform_owner_actor(uuid)') IS NULL THEN
    RAISE EXCEPTION 'public.assert_platform_owner_actor(uuid) must exist before applying Platform settings governance.'
      USING ERRCODE = '23514';
  END IF;
...
BEGIN
  ...
  IF v_section NOT IN ('feature-flags', 'global') THEN
    RAISE EXCEPTION 'Unsupported Platform configuration section: %', v_section
      USING ERRCODE = '23514';
  END IF;

  IF char_length(v_reason) < 5 THEN
    RAISE EXCEPTION 'A Platform configuration change reason of at least 5 characters is required.'
      USING ERRCODE = '23514';
  END IF;
  ...
  IF p_changes IS NULL
    ...
    RAISE EXCEPTION 'Platform configuration changes must be a non-empty JSON array of at most 100 entries.'
      USING ERRCODE = '23514';
  END IF;
  ...
  LOOP
    ...
    IF v_key = '' OR v_label = '' OR v_category = '' THEN
      RAISE EXCEPTION 'Each Platform configuration change requires key, label and category.'
        USING ERRCODE = '23514';
    END IF;

    IF v_section = 'feature-flags' THEN
      IF NOT (v_change ? 'enabled') OR lower(COALESCE(v_change->>'enabled', '')) NOT IN ('true', 'false') THEN
        RAISE EXCEPTION 'Feature flag % requires a boolean enabled value.', v_key
          USING ERRCODE = '23514';
      END IF;
      ...
      SELECT true, flag.is_enabled
      INTO v_exists, v_old_enabled
      ...
      INSERT INTO public.platform_feature_flags (
        key, label, description, category, is_enabled, updated_by
      )
      VALUES (
        v_key, v_label, v_description, v_category, v_enabled, p_actor_user_id
      )
      ...
      VALUES (
        ...
        jsonb_build_object(
          'key', v_key,
          'previous_enabled', v_old_enabled,
          'enabled', v_enabled,
          'category', v_category
        ),
        clock_timestamp()
    ...
    IF v_value_type NOT IN ('text', 'number', 'boolean') THEN
      RAISE EXCEPTION 'Platform setting % has an invalid value_type.', v_key
        USING ERRCODE = '23514';
    END IF;
    ...
    IF v_value_type = 'number' THEN
      ...
      EXCEPTION WHEN invalid_text_representation THEN
        RAISE EXCEPTION 'Platform setting % must contain a numeric value.', v_key
          USING ERRCODE = '23514';
      END;
    ELSIF v_value_type = 'boolean' AND lower(btrim(v_value)) NOT IN ('true', 'false') THEN
      RAISE EXCEPTION 'Platform setting % must contain true or false.', v_key
        USING ERRCODE = '23514';
    END IF;

    SELECT true, setting.value
    INTO v_exists, v_old_value
    ...
    INSERT INTO public.platform_settings (
      key, label, value, value_type, category, updated_by
    )
    VALUES (
      v_key, v_label, v_value, v_value_type, v_category, p_actor_user_id
    )
    ON CONFLICT (key) DO UPDATE
    SET
      label = EXCLUDED.label,
...
REVOKE ALL ON FUNCTION public.owner_update_platform_configuration(uuid, text, jsonb, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.owner_update_platform_configuration(uuid, text, jsonb, text) FROM anon;
REVOKE ALL ON FUNCTION public.owner_update_platform_configuration(uuid, text, jsonb, text) FROM authenticated;
...
IS 'Atomically applies changed Platform feature flags or global settings and writes one durable owner audit row per changed key.';
