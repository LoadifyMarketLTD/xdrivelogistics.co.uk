...
|---|---|---|---|---|---|---|
| GC-01-01 | Inventarul interactiv arată acoperire scăzută vs. obiectiv de audit complet | `docs/audit/platform-interactive-summary.json` (`CLOSED: 2`, `PARTIAL: 44`, `BROKEN: 17`, `DUPLICATE: 189`) | CRITICAL | risc major de fluxuri rupte în producție | execută remediere etapizată: `BROKEN` → `PLACEHOLDER` → `DUPLICATE` | ❌ |
| GC-01-02 | `robots` permite indexare globală, inclusiv suprafețe sensibile redirecționate | `app/robots.ts:7-11` (`allow: '/'`) + existență dashboard-uri protejate în `app/admin`, `app/super-admin`, `app/driver` | CRITICAL | risc SEO/indexare nedorită pentru URL-uri interne | `Disallow` explicit pentru suprafețe autentificate și pagini auth interne | ❌ |
...
|---|---|---|---|---|---|---|---|---|---|---|
| GC-EV-01 | `/home/runner/work/xdrivelogistics.co.uk/xdrivelogistics.co.uk/app/robots.ts:7-11` | robots policy | crawler/public | indexability check | private surfaces not indexable | allow global activ | CRITICAL | indexare accidentală URL private | introdu `disallow` pentru `/admin`, `/super-admin`, `/driver`, `/customer`, `/broker`, `/login`, `/register` | ❌ |
...
| GC-EV-03 | `/home/runner/work/xdrivelogistics.co.uk/xdrivelogistics.co.uk/docs/audit/platform-interactive-summary.json:2-14` | interactive routes consistency | toate rolurile | navigation matrix check | low broken/duplicate counts | BROKEN 17, DUPLICATE 189, inaccessiblePages 70 | CRITICAL | UX rupt și căi multiple către același flux | remediere tehnică + refactor navigație, apoi rerun audit script | ❌ |
