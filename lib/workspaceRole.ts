...
export const resolveWorkspaceRole = (user: WorkspaceUserLike | null | undefined): WorkspaceRole => {
  ...
  if (
    rawRole === 'fleet_operator' ||
    rawRole === 'fleet_manager' ||
    rawRole === 'fleet_admin' ||
    (appRole === 'company_admin' && membershipRole === 'owner')
  ) {
    return 'fleet_manager';
  }
  if (rawRole === 'dispatcher' || rawRole === 'operations_controller' || membershipRole === 'dispatcher') {
    return 'dispatcher';
  }
  if (
    rawRole === 'finance' ||
  ...
  if (membershipRole === 'admin' || appRole === 'company_admin') {
    return rawRole === 'carrier' ? 'carrier_admin' : 'company_admin';
  }
...
const CAPABILITIES: Record<WorkspaceRole, ReadonlySet<WorkspaceCapability>> = {
  platform_owner: new Set<WorkspaceCapability>([
    'platform.manage', ...ALL_COMPANY_MANAGEMENT, ...CARRIER_COMMERCIAL,
    'loads.create', 'loads.publish', 'loads.view.own', 'quotes.receive', 'quotes.compare',
    'quotes.award', 'jobs.execute', 'documents.own.manage', 'documents.verify',
    'invoices.customer.manage', 'payments.manage', 'margins.view',
  ]),
  company_owner: new Set<WorkspaceCapability>([
    ...ALL_COMPANY_MANAGEMENT, ...CARRIER_COMMERCIAL, 'loads.create', 'loads.publish',
    'loads.view.own', 'quotes.receive', 'quotes.compare', 'quotes.award',
    'invoices.customer.manage', 'payments.manage', 'margins.view',
  ]),
  ...
  broker: new Set<WorkspaceCapability>([
    'company.manage', 'loads.create', 'loads.publish', 'loads.view.own', 'quotes.receive',
    'quotes.compare', 'quotes.award', 'jobs.view', 'jobs.track', 'jobs.review_pod',
    'documents.company.manage', 'invoices.customer.manage', 'invoices.carrier.manage',
    'margins.view', 'incidents.manage', 'settings.manage',
  ]),
  customer: new Set<WorkspaceCapability>([
    'loads.create', 'loads.publish', 'loads.view.own', 'quotes.receive', 'quotes.compare',
    'quotes.award', 'jobs.view', 'jobs.track', 'jobs.review_pod',
    'invoices.customer.manage', 'settings.manage',
  ]),
  fleet_manager: new Set<WorkspaceCapability>([
    'jobs.view', 'jobs.allocate', 'jobs.dispatch', 'jobs.track', 'drivers.manage',
    'vehicles.manage', 'fleet.positions.view', 'fleet.maintenance.manage',
    'documents.company.manage', 'incidents.manage', 'settings.manage',
  ]),
  dispatcher: new Set<WorkspaceCapability>([
    'jobs.view', 'jobs.allocate', 'jobs.dispatch', 'jobs.track', 'jobs.review_pod',
    'drivers.manage', 'vehicles.manage', 'fleet.positions.view', 'incidents.manage',
  ]),
  ...
  owner_driver: new Set<WorkspaceCapability>([
    'loads.view.marketplace', 'quotes.submit', 'jobs.view', 'jobs.execute', 'jobs.track',
    'jobs.review_pod', 'vehicles.manage', 'documents.own.manage', 'invoices.carrier.manage',
  ]),
  finance: new Set<WorkspaceCapability>([
    'jobs.view', 'invoices.customer.manage', 'invoices.carrier.manage', 'payments.manage', 'margins.view',
  ]),
  compliance: new Set<WorkspaceCapability>([
    'drivers.manage', 'vehicles.manage', 'documents.company.manage', 'documents.verify', 'incidents.manage',
  ]),
  viewer: new Set<WorkspaceCapability>(['jobs.view']),
};

export const hasWorkspaceCapability = (role: WorkspaceRole, capability: WorkspaceCapability) =>
...
export const WORKSPACE_DEFINITIONS: Record<WorkspaceRole, WorkspaceDefinition> = {
  ...
  broker: { role: 'broker', label: 'Broker Workspace', subtitle: 'Customer loads, carrier sourcing and margin control', homeHref: '/broker', primaryAction: { label: 'Post Load', href: '/broker/post-load', capability: 'loads.create' }, nav: [
  ...
  customer: { role: 'customer', label: 'Customer Workspace', subtitle: 'Post, award and track your transport', homeHref: '/customer', primaryAction: { label: 'Post Load', href: '/customer/post-load', capability: 'loads.create' }, nav: [
  ...
  dispatcher: { role: 'dispatcher', label: 'Operations Workspace', subtitle: 'Allocate, monitor and recover daily jobs', homeHref: '/admin/operations-centre', nav: [
  ...
  owner_driver: { role: 'owner_driver', label: 'Owner Driver Workspace', subtitle: 'Find work, execute jobs and manage your business', homeHref: '/driver', primaryAction: { label: 'Find Loads', href: '/driver/loads', capability: 'loads.view.marketplace' }, nav: [
  ...
  viewer: { role: 'viewer', label: 'Read-only Workspace', subtitle: 'Approved operational visibility', homeHref: '/admin', nav: [
    { id: 'view', label: 'Read Only', items: [
  ...
  WORKSPACE_DEFINITIONS[role].nav
    .map((group) => ({
      ...
      items: group.items.filter(
        (item) => !item.capability || hasWorkspaceCapability(role, item.capability)
      ),
    }))
