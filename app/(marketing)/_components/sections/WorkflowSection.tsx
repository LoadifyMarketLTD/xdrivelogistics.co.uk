...
export function WorkflowSection() {
  return (
    <section id="flow" className="border-b border-slate-200 bg-[#f8fafc] px-4 py-14 sm:px-6">
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-8 lg:grid-cols-[360px_1fr] lg:items-start">
          <div>
            <span className="text-xs font-semibold uppercase tracking-[0.14em] text-[#1d4ed8]">Flow</span>
            <h2 className="mt-2 text-3xl font-black leading-tight text-[#0f172a] sm:text-4xl">
              The homepage now follows the record as it moves forward.
            </h2>
            <p className="mt-4 text-sm leading-6 text-slate-600">
              Each step below is based on product surfaces that already exist or are clearly adjacent to the live workflow.
            </p>
          </div>

          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_16px_40px_-34px_rgba(15,23,42,0.6)]">
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {workflow.map(({ title, detail, proof, icon: Icon }, index) => (
                <article key={title} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  <p className="mt-3 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#1d4ed8]">{proof}</p>
                </article>
              ))}
            </div>
          </div>
