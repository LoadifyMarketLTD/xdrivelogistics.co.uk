...
const X = { blue:'#1D57D8', navy:'#0B2F6B', orange:'#F5A300', white:'#FFFFFF', charcoal:'#1A1F2B', light:'#F4F6F8', border:'#E5E7EB', red:'#D92D20' } as const;
...
export function ActionConfirmModal({open,title,description,confirmLabel,cancelLabel='Cancel',danger=true,reasonLabel='Reason (required)',reasonRequired=true,reasonPlaceholder='Describe the reason for this action…',submitting=false,onConfirm,onCancel}:ActionConfirmModalProps){
 ...
 return <div role='dialog' aria-modal='true' aria-labelledby='acm-title' style={{position:'fixed',inset:0,zIndex:1000,backgroundColor:'rgba(74,74,74,.24)',display:'flex',alignItems:'center',justifyContent:'center',padding:'24px'}} onClick={e=>{if(e.target===e.currentTarget)onCancel();}}>
  <div style={{backgroundColor:X.white,border:`1px solid ${X.border}`,borderTop:`4px solid ${confirmColor}`,borderRadius:'8px',padding:'24px',width:'100%',maxWidth:'520px',boxShadow:'0px 2px 6px rgba(0,0,0,0.08)',fontFamily:'Inter, Arial, sans-serif',color:X.charcoal}}>
   <h2 id='acm-title' style={{margin:'0 0 8px',fontFamily:'Inter, Arial, sans-serif',fontSize:'20px',lineHeight:1.25,fontWeight:700,color:X.navy}}>{title}</h2>
   <div style={{margin:'0 0 16px',fontSize:'14px',color:X.charcoal,lineHeight:1.5}}>{description}</div>
   <label htmlFor='acm-reason' style={{display:'block',marginBottom:'6px',fontFamily:'Inter, Arial, sans-serif',fontSize:'14px',fontWeight:700,color:X.charcoal}}>{reasonLabel}{reasonRequired&&<span style={{color:X.red,marginLeft:'3px'}}>*</span>}</label>
   <textarea id='acm-reason' value={reason} onChange={e=>setReason(e.target.value)} rows={4} placeholder={reasonPlaceholder} disabled={submitting} style={{width:'100%',boxSizing:'border-box',backgroundColor:X.white,border:`1px solid ${reasonRequired&&reason.trim().length>0&&reasonTooShort?X.red:X.border}`,borderRadius:'8px',padding:'24px',color:X.charcoal,fontSize:'14px',lineHeight:1.5,resize:'vertical',outlineColor:X.blue,fontFamily:'Inter, Arial, sans-serif'}}/>
   ...
   <div style={{display:'flex',gap:'10px',marginTop:'18px',justifyContent:'flex-end'}}><button type='button' onClick={onCancel} disabled={submitting} style={{minHeight:'40px',padding:'0 14px',borderRadius:'8px',border:`1px solid ${X.border}`,backgroundColor:X.white,color:X.blue,fontFamily:'Inter, Arial, sans-serif',fontSize:'14px',fontWeight:700,cursor:submitting?'not-allowed':'pointer'}}>{cancelLabel}</button><button type='button' onClick={()=>{if(!isDisabled)onConfirm(reason.trim());}} disabled={isDisabled} style={{minHeight:'40px',padding:'0 14px',borderRadius:'8px',border:`1px solid ${confirmColor}`,backgroundColor:confirmColor,color:X.white,fontFamily:'Inter, Arial, sans-serif',fontWeight:700,fontSize:'14px',cursor:isDisabled?'not-allowed':'pointer',opacity:isDisabled?.55:1}}>{submitting?'…':confirmLabel}</button></div>
  </div>
 </div>;
}
