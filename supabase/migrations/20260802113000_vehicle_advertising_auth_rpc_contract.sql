...
--   - Recreate the legacy 5-argument function only if a controlled internal caller still requires it

BEGIN;

DO $$
BEGIN
  IF to_regprocedure('public.set_vehicle_advertising_state(uuid, uuid, text, text, jsonb)') IS NOT NULL THEN
    REVOKE ALL ON FUNCTION public.set_vehicle_advertising_state(uuid, uuid, text, text, jsonb) FROM PUBLIC;
    REVOKE ALL ON FUNCTION public.set_vehicle_advertising_state(uuid, uuid, text, text, jsonb) FROM authenticated;
    REVOKE ALL ON FUNCTION public.set_vehicle_advertising_state(uuid, uuid, text, text, jsonb) FROM service_role;
  END IF;
END;
$$;

DROP FUNCTION IF EXISTS public.set_vehicle_advertising_state(uuid, uuid, text, text, jsonb);
