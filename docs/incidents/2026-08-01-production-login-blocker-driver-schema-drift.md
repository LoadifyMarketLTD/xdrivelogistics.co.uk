...
| `owner_decide_fraud_review_case(uuid,uuid,text,text)` | Raw Production function output for the exact signature is still missing. Live status is not yet proven. | Repo migration `20260730100000` omits `target_type` and `target_id`. `20260801130000` is now a **SUPERSEDED NO-OP**; `20260801163000` is the only remaining candidate patch and is still blocked pending live evidence. | **BLOCKED (PRODUCTION) / DIVERGENT (REPO-CANONICAL)** | Run the exact single-signature function-body query and the `fraud_review_cases` base-table query. Then apply the documented decision matrix: function absent = N/A; already patched = N/A; table absent/not base table = N/A + separate drift incident; otherwise stage-validate `20260801163000` only. |
...
| Returns one row whose body does **not** include `target_type` and `fraud_review_cases` is a `BASE TABLE` | Same bug as marketplace function; repo-canonical drift appears to exist live. | Treat `20260801163000_p0_fix_fraud_review_case_audit_target_type.sql` as a **staging/disposable validation candidate only**. Do not apply directly to Production. |
| Returns zero rows | Function does not exist with this exact signature. | Classify `20260801163000` as **NOT APPLICABLE** for Production and do not patch. |
| `fraud_review_cases` is absent or not a `BASE TABLE` | The function patch cannot be compiled safely against the current live schema. | Classify `20260801163000` as **NOT APPLICABLE**, open a separate schema-drift incident, and do not patch. |

`owner_audit_log.target_type` remediation status summary: one confirmed broken (marketplace — patch staged), one blocked pending raw Production function/table evidence (fraud review — `20260801163000` only), and one partially aligned / optional later enrichment (`set_company_status_governance` — `20260801153000` superseded no-op; `20260801160500` only becomes relevant if the column-nullability gate proves extra audit fields are still needed).
