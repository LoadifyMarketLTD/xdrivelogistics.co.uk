-- Forward migration: harden review_onboarding_application_atomic against
-- PL/pgSQL name-resolution ambiguity on company_id during membership upsert.
--
-- Root cause: `ON CONFLICT (company_id, user_id)` is ambiguous when executed
-- inside a SECURITY DEFINER function whose scope also exposes the local
-- variable `v_company_id`.  PostgreSQL raises "column reference is ambiguous"
-- for company-linked onboarding approvals.
--
-- Fix: replace the implicit column-list target with the explicit named
-- constraint, which is unambiguous regardless of execution context.
