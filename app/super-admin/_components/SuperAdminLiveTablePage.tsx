  ...
  return (
    <ProtectedRoute allowedRoles={['owner']}>
      <div style={{ padding: '12px 16px', maxWidth: '1480px', margin: '0 auto' }}>
        ...
        {note && !loading && (
          <div style={{ backgroundColor: 'rgba(245,158,11,0.08)', border: `1px solid rgba(245,158,11,0.3)`, borderRadius: '4px', padding: '0.65rem 0.9rem', color: THEME.accent, fontSize: '0.8rem', marginBottom: '1rem' }}>
            ℹ️ {note}
          </div>
        )}
        ...
        {summary && !loading && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '0.6rem', marginBottom: '1rem' }}>
            {Object.entries(summary).map(([key, value]) => (
              <div key={key} style={{ backgroundColor: THEME.cardBg, border: `1px solid ${THEME.cardBorder}`, borderRadius: '4px', padding: '0.6rem 0.75rem' }}>
        ...
        <div style={{ backgroundColor: THEME.cardBg, border: `1px solid ${THEME.cardBorder}`, borderRadius: '4px', overflow: 'hidden' }}>
          {loading ? (
