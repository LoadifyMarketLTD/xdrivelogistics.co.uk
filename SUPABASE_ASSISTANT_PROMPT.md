...
>    column presence, then run **`supabase/complete_fix.sql`** to apply all missing objects.
>    Both files contain pure SQL and can be pasted directly into the SQL Editor.
>
> ⚠️ **Do not run this `.md` file directly in the SQL Editor** — it is a Markdown document,
> not a SQL file. Use the dedicated `.sql` files in the `supabase/` folder instead.
