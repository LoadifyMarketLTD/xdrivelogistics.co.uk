...
export async function performNotificationRetry({notificationId,reason,getAuthHeaderImpl=getAuthHeader,fetchImpl=fetch,onSuccess}:{notificationId:string;reason:string;getAuthHeaderImpl?:typeof getAuthHeader;fetchImpl?:typeof fetch;onSuccess?:()=>void|Promise<void>}):Promise<RetryFeedback>{
 const normalizedReason=reason.trim(); if(normalizedReason.length<5)return{tone:'error',message:'A retry reason of at least 5 characters is required.'};
 ...
 try{response=await fetchImpl('/api/super-admin/platform',{method:'PATCH',headers:{'Content-Type':'application/json',Authorization:auth},body:JSON.stringify({section:'notifications',action:'retry',notificationId,reason:normalizedReason})});}catch{return{tone:'error',message:'Notification retry is currently unavailable.'};}
 await response.json().catch(()=>null); if(!response.ok)return{tone:'error',message:'Notification retry is currently unavailable.'}; await onSuccess?.(); return{tone:'success',message:'Retry queued and audited.'};
}
