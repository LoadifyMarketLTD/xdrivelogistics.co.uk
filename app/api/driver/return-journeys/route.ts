...
export async function GET(request: NextRequest) {
  ...
  const [companiesResult, driversResult] = await Promise.all([
    companyIds.length ? supabaseAdmin.from('companies').select('id,name,xd_id,phone').in('id', companyIds) : Promise.resolve({ data: [], error: null }),
  ...
  const mapped = rows.map((row) => {
    ...
    return {
      ...
      member: {
        ...
        code: cleanText(company.xd_id, 80) || null,
