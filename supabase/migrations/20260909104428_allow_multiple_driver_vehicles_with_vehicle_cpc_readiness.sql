...
BEGIN
  ...
  IF NOT FOUND
     ...
     OR COALESCE(v_driver.driver_type, '') NOT IN ('owner_driver', 'company_driver') THEN
  ...
  IF v_onboarding_id IS NULL
     ...
     OR (COALESCE(v_driver.driver_type, '') = 'company_driver' AND COALESCE(v_onboarding_account_type, '') NOT IN ('individual_driver', 'company_driver')) THEN
  ...
  ORDER BY
    ...
    COALESCE(v.max_weight_kg, 999999) ASC,
    v.id
  LIMIT 1;
