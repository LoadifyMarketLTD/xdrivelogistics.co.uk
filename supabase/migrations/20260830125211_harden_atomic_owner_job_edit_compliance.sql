...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Only an authorised member of the load-owning company can edit this load.' USING ERRCODE = '42501';
  END IF;
  ...
  IF v_status NOT IN ('draft', 'received', 'posted')
    ...
    RAISE EXCEPTION 'Only pre-award loads can be edited.' USING ERRCODE = '23514';
  END IF;
  ...
  IF v_job.awarded_carrier_company_id IS NOT NULL
    ...
    RAISE EXCEPTION 'Active, awarded or assigned loads cannot be edited.' USING ERRCODE = '23514';
  END IF;

  IF EXISTS (SELECT 1 FROM public.job_bids WHERE job_id = p_job_id) THEN
    RAISE EXCEPTION 'Carrier quotes already exist for this load. Changing the transport terms would make those quotes stale.' USING ERRCODE = '23514';
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM public.job_commercial_agreements WHERE job_id = p_job_id)
    ...
    RAISE EXCEPTION 'This load already has protected commercial or execution history.' USING ERRCODE = '23514';
  END IF;

  IF EXISTS (
    SELECT 1 FROM public.job_stops s
    WHERE s.job_id = p_job_id
      AND (lower(coalesce(s.status, 'pending')) <> 'pending' OR s.arrived_at IS NOT NULL OR s.completed_at IS NOT NULL)
  ) THEN
    RAISE EXCEPTION 'This load already has progressed stop history.' USING ERRCODE = '23514';
  END IF;

  IF EXISTS (
    SELECT 1 FROM jsonb_to_recordset(p_stops) AS s(sequence integer, stop_type text, address text)
  ...
  IF v_status = 'posted' OR v_current_status = 'posted' THEN
    IF NOT coalesce(p_publish, true) THEN
      RAISE EXCEPTION 'A posted load cannot be moved back to draft by editing; cancel it instead.' USING ERRCODE = '23514';
    END IF;
  ...
  -- Posted -> posted edits no longer toggle visibility merely to re-trigger the
  -- publish gate. Run the same publish compliance contract explicitly instead.
  IF v_final_status = 'posted' THEN
    v_issues := public.company_compliance_issues(v_job.company_id, 'publish');
  ...
  IF jsonb_array_length(p_stops) > 0 THEN
    INSERT INTO public.job_stops(job_id, sequence, stop_type, address, postcode, contact_name, contact_phone, window_start, instructions)
    SELECT p_job_id, s.sequence, lower(s.stop_type), s.address, s.postcode, s.contact_name, s.contact_phone, s.window_start, s.instructions
  ...
  INSERT INTO public.owner_audit_log(
    target_type, target_id, target_name, metadata, actor_user_id,
    target_company_id, action_type, old_status, new_status, reason
  )
  VALUES (
    'job', v_job.id, 'XDL-' || upper(left(v_job.id::text, 8)),
    jsonb_build_object('source', 'workspace_owner_edit', 'previous_current_status', v_current_status, 'stop_count', jsonb_array_length(p_stops)),
    p_actor_user_id, v_job.company_id, 'exchange_load_edited_without_bids',
    v_status, v_final_status, 'Load terms edited before bids, award or execution history.'
  );

  RETURN jsonb_build_object('id', p_job_id, 'status', v_final_status, 'current_status', v_final_status);
END;
$function$;
