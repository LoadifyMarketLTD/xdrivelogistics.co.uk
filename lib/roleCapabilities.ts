...
export const isCapabilityAllowedForPath = (
  ...
  if (pathname.startsWith('/transport-broker')) return role === 'broker' || role === 'owner' || role === 'company_admin';
  if (pathname.startsWith('/customer')) return role === 'customer';
  if (pathname.startsWith('/shipper')) return role === 'customer' || role === 'owner';
  if (pathname.startsWith('/owner-operator')) return role === 'driver' || role === 'company_admin' || role === 'company_staff' || role === 'broker' || role === 'owner';
  if (pathname.startsWith('/fleet-operator')) return role === 'company_admin' || role === 'company_staff' || role === 'broker' || role === 'owner';
