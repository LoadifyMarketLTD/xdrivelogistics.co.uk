...
export class ApiRequestError extends Error {
  ...
  constructor(message: string, status: number) {
    // Keep the status in the persisted message as well as on the Error object.
    // Offline replay stores only lastError text, so this lets the queue retain
    // retry semantics across the catch boundary and an app restart.
