...
export default function WonWorkPage() {
  ...
  const pipelineCounts = {
    active: jobs.filter((j) => ['allocated', 'in_transit'].includes(j.status)).length,
    pending: jobs.filter((j) => ['draft', 'posted'].includes(j.status)).length,
  ...
  return (
    <ProtectedRoute allowedRoles={['driver']}>
      <DriverWorkspaceShell
        subtitle="Jobs your company has won on the exchange — from allocation through to completion."
      >
        <h2 style={{ margin: '0 0 1rem', fontSize: '1.35rem', fontWeight: 700, color: '#0f172a' }}>Won Work</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '0.65rem', marginBottom: '1rem' }}>
          {[
            ...
            { label: 'Total won',  value: pipelineCounts.total,     color: '#374151', bg: '#f3f4f6' },
          ].map((item) => (
        ...
        {loading ? (
          <div style={{ color: '#64748b', padding: '2rem', textAlign: 'center' }}>Loading won work…</div>
        ) : jobs.length === 0 ? (
          <div style={{ ...card, textAlign: 'center', padding: '2.5rem' }}>
            <div style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>🏆</div>
            <div style={{ fontWeight: 700, color: '#0f172a', marginBottom: '0.3rem' }}>No won work yet</div>
            <div style={{ fontSize: '0.84rem', color: '#64748b' }}>
              Win jobs by submitting competitive quotes on the exchange.
            </div>
            <button
                ...
                <div key={job.id} style={{ ...card, borderLeft: `3px solid ${statusStyle.color}` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '0.65rem' }}>
                    ...
                    {job.budget_amount != null && (
                      <span style={{ fontSize: '1.05rem', fontWeight: 800, color: '#15803d' }}>£{job.budget_amount.toFixed(2)}</span>
                    )}
                  </div>
