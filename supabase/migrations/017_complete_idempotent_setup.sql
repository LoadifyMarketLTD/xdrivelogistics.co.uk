-- ============================================================
-- 017_complete_idempotent_setup.sql
--
-- XDrive Logistics — complete, idempotent database setup.
--
-- Run this file in the Supabase SQL Editor to ensure every
-- table, column, enum, function, trigger, RLS policy, and
-- index required by the site exists.
--
-- Safe to re-run on any database state:
--   - CREATE TABLE  IF NOT EXISTS
--   - ADD COLUMN    IF NOT EXISTS
--   - CREATE INDEX  IF NOT EXISTS
--   - CREATE POLICY only inside a guard that checks pg_policies
--   - CREATE OR REPLACE FUNCTION
--   - NOTIFY pgrst at the end reloads the PostgREST schema cache
-- ============================================================
...
DO $$ BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tracking_event_type'
    ...
    CREATE TYPE public.tracking_event_type AS ENUM
      ('created','allocated','driver_en_route','arrived_pickup','collected',
       'in_transit','arrived_delivery','delivered','failed','cancelled','note');
  END IF;
...
-- drivers: driver-app columns  (also adds display_name for any old schema without it)
-- Note: display_name is NOT NULL in CREATE TABLE (new installs). The ALTER omits NOT NULL
-- deliberately — PostgreSQL forbids ADD COLUMN ... NOT NULL without a DEFAULT when the table
-- already contains rows. Set any NULLs to a placeholder after running this if needed:
--   UPDATE public.drivers SET display_name = 'Unknown' WHERE display_name IS NULL;
...
-- driver_locations
ALTER TABLE public.driver_locations ADD COLUMN IF NOT EXISTS company_id uuid REFERENCES public.companies(id) ON DELETE SET NULL;
ALTER TABLE public.driver_locations ADD COLUMN IF NOT EXISTS updated_at timestamptz DEFAULT now();

-- return_journeys
ALTER TABLE public.return_journeys ADD COLUMN IF NOT EXISTS status text DEFAULT 'available';
...
-- ── 5. UNIQUE CONSTRAINT on invoice number per company ─────────
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    WHERE  constraint_type   = 'UNIQUE'
      ...
      AND  constraint_name   = 'invoices_company_invoice_number_unique'
  ) THEN
...
BEGIN
  ...
  -- 2. created a company without a membership row?
  ...
  IF v_company_id IS NOT NULL THEN
    ...
    ON CONFLICT (company_id, user_id) DO UPDATE
      SET status = 'active', role_in_company = 'owner';
    RETURN v_company_id;
  END IF;

  -- 3. first time – auto-provision a company
...
-- Trigger function: keep amount <-> bid_price_gbp and bidder_id <-> bidder_user_id in sync
...
BEGIN
  ...
  ELSIF NEW.amount IS NULL AND NEW.bid_price_gbp IS NOT NULL THEN
    NEW.amount := NEW.bid_price_gbp;
  ELSIF NEW.bid_price_gbp IS NOT NULL AND NEW.amount IS NOT NULL
  ...
  ELSIF NEW.bidder_user_id IS NULL AND NEW.bidder_id IS NOT NULL THEN
    NEW.bidder_user_id := NEW.bidder_id;
  ELSIF NEW.bidder_id IS NOT NULL AND NEW.bidder_user_id IS NOT NULL
...
-- Trigger function: auto-update invoices.updated_at
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='companies' AND policyname='companies_insert_authenticated') THEN
...
-- ── 11. HEALTH CHECK (optional – review output, expect all "exists") ───────
--
-- Uncomment the SELECT below to verify every required column is present.
-- It returns one row per column: status = "exists" or "MISSING".
--
-- SELECT
--   e.table_name,
--   e.column_name,
...
--     ('profiles','role'),('profiles','company_id'),('profiles','is_driver'),
--     ('companies','id'),('companies','name'),('companies','address_line1'),
--     ('companies','city'),('companies','postcode'),
--     ('drivers','id'),('drivers','company_id'),('drivers','display_name'),
--     ('drivers','login_pin'),('drivers','app_access'),('drivers','last_app_login'),
--     ('vehicles','id'),('vehicles','company_id'),('vehicles','type'),
--     ('vehicles','reg_plate'),('vehicles','has_tail_lift'),
--     ('jobs','id'),('jobs','company_id'),('jobs','assigned_driver_id'),
--     ('jobs','status'),('jobs','pickup_location'),('jobs','delivery_location'),
--     ('jobs','budget_amount'),('jobs','collection_photo_url'),
--     ('jobs','delivery_photos'),('jobs','delivery_signature_data'),
--     ('jobs','status_history'),('jobs','driver_notes'),('jobs','client_signature_name'),
--     ('quotes','id'),('quotes','company_id'),('quotes','customer_name'),
--     ('quotes','amount'),('quotes','status'),
--     ('invoices','id'),('invoices','company_id'),('invoices','invoice_number'),
--     ('invoices','job_ref'),('invoices','status'),('invoices','amount'),
--     ('invoices','vat_rate'),('invoices','payment_terms'),
