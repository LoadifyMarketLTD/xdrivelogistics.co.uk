...
export async function GET(request: NextRequest) {
  ...
  const { data: profile, error: profileError } = await supabaseAdmin.from('profiles').select('role, status').eq('user_id', authData.user.id).maybeSingle();
  ...
  if (companyId) {
    const { data: membership, error: membershipError } = await supabaseAdmin.from('company_memberships').select('role_in_company, status').eq('company_id', companyId).eq('user_id', authData.user.id).eq('status', 'active').maybeSingle();
    ...
    const { data: activeMemberships, error: activeMembershipError } = await supabaseAdmin.from('company_memberships').select('company_id, role_in_company').eq('user_id', authData.user.id).eq('status', 'active').limit(1);
    if (activeMembershipError) return json(500, { error: activeMembershipError.message });
    if ((activeMemberships ?? []).length > 0) {
      return json(200, { excluded: false, delegated: true, reason: 'company_billing_managed_by_owner_admin', subscription: null });
    }
  }

  let query = supabaseAdmin.from('platform_membership_subscriptions').select('plan_id, status, trial_started_at, trial_ends_at, current_period_end, cancel_at_period_end, contract_terms_version, contract_accepted_at, stripe_customer_id, stripe_subscription_id');
  query = companyId ? query.eq('company_id', companyId) : query.eq('user_id', authData.user.id).is('company_id', null);
  ...
  if (!subscription) return json(200, { excluded: false, delegated: false, reason: 'membership_missing', subscription: null });
