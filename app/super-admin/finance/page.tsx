  ...
  return <ProtectedRoute allowedRoles={['owner']}><SuperAdminPage>
    ...
    {!error ? <>
      ...
      <SuperAdminSectionCard title="Settlement activity · last 7 days" description="Actual payment-history amounts only; no synthetic trend or forecast.">
        {loading ? <div>Loading verified settlement history…</div> : weekly.length===0 ? <SuperAdminNotice tone="unavailable">No settlement history is available for this period.</SuperAdminNotice> : <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(120px,1fr))',gap:12}}>{weekly.map((item)=><div key={item.date} style={{padding:12,border:'1px solid #E5E7EB',borderRadius:8}}><strong>{money(item.amount)}</strong><div style={{marginTop:6,fontSize:12,color:'#667085'}}>{new Date(`${item.date}T00:00:00Z`).toLocaleDateString('en-GB',{weekday:'short',day:'2-digit',month:'short'})}</div></div>)}</div>}
      </SuperAdminSectionCard>
      <SuperAdminSectionCard title="Finance workspaces" description="Drill into canonical ledgers and evidence."><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(170px,1fr))',gap:12}}>{links.map(([href,label])=><Link key={href} href={href} data-card="enterprise" style={{padding:14,border:'1px solid #E5E7EB',borderRadius:8,textDecoration:'none',fontWeight:800,color:'#0b2f6b',background:'#FFFFFF'}}>{label}</Link>)}</div></SuperAdminSectionCard>
