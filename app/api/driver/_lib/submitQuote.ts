...
export type DriverQuoteInput = {
  jobId: string;
  /** Final quoted total, including VAT when the Driver elected to apply it. */
  amount: number;
  message: string;
  /** Base transport price before explicit extras/VAT. */
  baseAmount?: number | null;
  /** Structured extras before VAT. */
  additionalExtrasGbp?: number | null;
  /** Driver-declared time to collection after acceptance. */
...
export async function submitDriverQuote(
  ...
  const collectWithinMinutes = input.collectWithinMinutes == null
    ? null
    : Math.round(Number(input.collectWithinMinutes));
  ...
  if (!Number.isFinite(totalAmount) || totalAmount <= 0 || totalAmount > 1_000_000) {
    return { ok: false, status: 400, error: 'Enter a valid quote total.' };
  }
  if (!Number.isFinite(baseAmount) || baseAmount <= 0 || baseAmount > 1_000_000) {
    return { ok: false, status: 400, error: 'Enter a valid base quote amount.' };
  }
  ...
  if (baseAmount + additionalExtrasGbp > totalAmount + 0.01) {
    return { ok: false, status: 400, error: 'Quote total cannot be lower than the base amount plus extras.' };
  }
  ...
  const vehicleResult = await loadCanonicalVehicleSnapshot(
    supabaseAdmin,
    driver,
    eligibility.operational.canonicalVehicleId,
  );
