...
function matchesTab(job: WorkspaceJob, tab: JobsTab) {
  ...
  if (tab === 'all') return Boolean(job.awarded_carrier_company_id) || ACTIVE.has(status) || ['awarded', 'delivered', 'completed'].includes(status);
  if (tab === 'awarded') return status === 'awarded' || Boolean(job.awarded_carrier_company_id) && !ACTIVE.has(status) && !['delivered', 'completed'].includes(status);
  if (tab === 'active') return ACTIVE.has(status);
  if (tab === 'loaded') return ['loaded', 'collected'].includes(status);
  if (tab === 'in_transit') return ['in_transit', 'on_my_way_to_delivery', 'on_site_delivery'].includes(status);
  ...
  return (
    <PageFrame>
      <PageHeader
        ...
        title="Jobs"
        description="Monitor awarded carrier work from collection through POD and completion in one operational board."
        ...
        <main style={{ minWidth: 0 }}>
          ...
          {rows.length === 0 ? (
            <div className="workspace-panel" style={{ border: '1px solid var(--ws-border)', background: '#fff' }}>
              <EmptyState compact title="No matching jobs" description="Awarded and active carrier work will appear here." />
            </div>
          ) : (
                ...
                return (
                  <article className="workspace-operational-row" key={job.id} data-state={statusOf(job)}>
                    ...
                    <div className="workspace-record-meta">
                      ...
                      <span>{job.assigned_driver_id ? 'Driver allocated' : 'Driver allocation not exposed'}</span>
                    ...
                    {open && (
                      <div className="workspace-record-details">
                        ...
                        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 5 }}>
                          ...
                          {['delivered', 'completed'].includes(statusOf(job)) && <ActionButton tone="secondary" onClick={() => router.push('/broker/pod-review')}>POD review</ActionButton>}
                        </div>
                      </div>
                    )}
                  </article>
