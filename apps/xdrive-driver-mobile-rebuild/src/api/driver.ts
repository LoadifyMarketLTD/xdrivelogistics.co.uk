...
export function mapJob(row: RawJob): DriverJob {
  ...
  return {
    ...
    pickupTiming: text(row.pickupTiming ?? row.pickup_time_slot, '') || undefined,
    deliveryTiming: text(row.deliveryTiming ?? row.delivery_time_slot, '') || undefined,
...
export async function fetchAvailableJobs(options: { destinationMode?: boolean; radiusMiles?: 10 | 20 | 30 } = {}): Promise<AvailableJobsResult> {
  ...
  const jobs = (payload.jobs ?? []).map((row) => {
    ...
    return mapJob({
      ...
      pickupTiming: pickup?.timeSlot ?? pickup?.collectionFrom,
      deliveryTiming: delivery?.timeSlot ?? delivery?.deliveryFrom,
