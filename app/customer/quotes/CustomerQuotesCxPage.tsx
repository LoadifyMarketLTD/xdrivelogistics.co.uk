    ...
    >
      ...
      >
        <header style={{ padding: '10px 12px', borderBottom: '1px solid #e2e8f0', background: '#f4f6f8' }}>
          <strong id="customer-award-confirmation-title" style={{ display: 'block', fontSize: 14, lineHeight: '20px', color: '#0f172a' }}>
            Confirm carrier award
          </strong>
          <span style={{ display: 'block', marginTop: 2, fontSize: 11, lineHeight: '15px', color: '#64748b' }}>
            Review the selected carrier, price and load before committing the booking.
          </span>
        </header>
        ...
        <div style={{ padding: 12, display: 'grid', gap: 10 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', border: '1px solid #e2e8f0', borderRadius: 4, overflow: 'hidden' }}>
            {[
              ...
              ['Agreed quote', money(amount, candidate.bid.currency ?? 'GBP')],
          ...
          {candidate.bid.message ? (
            <div style={{ padding: 9, border: '1px solid #e2e8f0', borderRadius: 4, background: '#f8fafc', color: '#334155', fontSize: 11, lineHeight: '16px' }}>
              <strong>Carrier message:</strong> {candidate.bid.message}
            </div>
          ) : null}
...
export default function CustomerQuotesCxPage() {
  ...
  const [candidate, setCandidate] = useState<AwardCandidate | null>(null);
  ...
  useEffect(() => {
    ...
    const loadIdentities = async () => {
      ...
      if (!token) {
        setIdentityError('Member profiles are unavailable until the session is refreshed.');
        return;
      }
      ...
      if (cancelled) return;
      if (!response.ok) {
        setIdentityError(payload.error ?? 'Bidder member profiles could not be resolved.');
        return;
      }
  ...
  useEffect(() => {
    ...
    const keydown = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && !working) setCandidate(null);
    };
      ...
      .map((job) => ({
        ...
        quotes: allQuotes
          ...
          .filter((bid) => {
            ...
            return `${identity?.displayName ?? ''} ${identity?.companyName ?? ''} ${identity?.personName ?? ''} ${identity?.companyId ?? ''} ${identity?.driverId ?? ''} ${bid.companies?.name ?? ''} ${bid.company_id ?? ''}`
              .toLowerCase()
              .includes(carrierNeedle);
          })
  ...
  return (
    <PageFrame>
      ...
      <div className="workspace-board-layout">
        <aside className="workspace-filter-rail" aria-label="Customer quote filters">
          <div className="workspace-filter-rail__header">Search Quotes</div>
          <div className="workspace-filter-rail__body">
            <label>LOAD ID / REF<input value={reference} onChange={(event) => setReference(event.target.value)} placeholder="XDrive or customer reference" /></label>
            <label>CARRIER / MEMBER<input value={carrierSearch} onChange={(event) => setCarrierSearch(event.target.value)} placeholder="Company, owner driver or member ID" /></label>
            <label>STATUS<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as typeof statusFilter)}><option value="all">All quote activity</option><option value="submitted">Awaiting decision</option><option value="accepted">Accepted</option><option value="rejected">Rejected</option></select></label>
            <ActionButton tone="secondary" onClick={() => { setReference(''); setCarrierSearch(''); setStatusFilter('all'); }}>Clear</ActionButton>
          </div>
        </aside>
        ...
        <main style={{ minWidth: 0 }}>
          <div className="workspace-tab-strip" style={{ display: 'flex', overflowX: 'auto', marginBottom: 4 }}>
            {(['all', 'submitted', 'accepted', 'rejected'] as const).map((status) => (
              <button key={status} type="button" data-active={statusFilter === status ? 'true' : 'false'} onClick={() => setStatusFilter(status)}>
                {status === 'all' ? 'All' : status === 'submitted' ? 'Awaiting Decision' : status[0].toUpperCase() + status.slice(1)} {counts[status]}
              </button>
            ))}
          </div>
          <div className="workspace-record-meta" style={{ justifyContent: 'space-between' }}>
            <span><strong>{grouped.length}</strong> load{grouped.length === 1 ? '' : 's'} with matching quotes</span>
            <span>Lowest visible price shown first per load · Award requires confirmation</span>
          </div>

          {grouped.length === 0 ? (
            <div className="workspace-panel"><EmptyState title={data.loading ? 'Loading quotes…' : 'No quotes in this view'} description="Carrier responses appear here after a load is published." /></div>
          ) : grouped.map(({ job, quotes }) => (
            <section key={job.id} className="workspace-panel" style={{ marginBottom: 8 }}>
              <div className="workspace-record-meta" style={{ justifyContent: 'space-between' }}>
                <span><strong>{routeLabel(job)}</strong> · Pickup {when(job.pickup_datetime)} · XDrive XDL-{job.id.slice(0, 8).toUpperCase()}</span>
              ...
              <DataTable
                ...
                rows={quotes.map((bid, index) => {
                  ...
                  const displayName = isOwnerDriverBid
                    ? (identity?.personName || identity?.displayName || 'Owner Driver')
                    : (identity?.companyName || bid.companies?.name || identity?.displayName || 'Carrier');
                  return [
                    ...
                    bid.status === 'submitted' ? (
                      <span key="actions" style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                        <ActionButton
                          ...
                          onClick={() => setCandidate({ bid, job, identity, displayName, isOwnerDriverBid })}
                        >
                          Review & Award
                        </ActionButton>
      ...
      {candidate ? (
        <AwardConfirmation
          candidate={candidate}
          working={working === candidate.bid.id}
          onCancel={() => setCandidate(null)}
          onConfirm={() => void award(candidate.bid.id)}
        />
      ) : null}
    </PageFrame>
  );
