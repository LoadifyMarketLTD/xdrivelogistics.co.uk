...
export async function GET(request: NextRequest) {
  ...
  if (membershipError) return json(500, { error: 'Unable to verify company access.' });
  const companyIds = [...new Set((memberships ?? []).map((row) => String(row.company_id)).filter(Boolean))];
  if (!companyIds.length) return json(200, { identities: [] });

  // Only bidder identities attached to jobs owned by one of the caller's active
  // companies are visible. This does not expose unrelated Marketplace members.
