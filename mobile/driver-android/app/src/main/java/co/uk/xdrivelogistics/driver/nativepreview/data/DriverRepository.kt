...
class DriverRepository(private val api:XDriveApi) {
    ...
    suspend fun alerts(): Result<List<NativeAlert>> = parse(api.getResources()) { root ->
        val resources=root.optJSONObject("resources")?:JSONObject(); val a=resources.optJSONArray("alerts")?:JSONArray(); buildList { for(i in 0 until a.length()){ val x=a.optJSONObject(i)?:continue; val p=x.optJSONObject("payload")?:JSONObject(); val title=p.optString("title").ifBlank { x.optString("event_type","Activity").replace('_',' ') }; val body=p.optString("message").ifBlank { p.optString("body") }; add(NativeAlert(x.optString("id"),title,body,x.optString("event_type"),x.str("created_at"),p.optString("read_at").isBlank(),x.str("entity_type")?:p.str("entity_type")?:p.str("entityType"),x.str("entity_id")?:p.str("entity_id")?:p.str("entityId")?:p.str("job_id")?:p.str("jobId"),x.str("status")?:p.str("status"),p.toString(),p.optString("saved_at").isNotBlank(),p.optString("deleted_at").isNotBlank(),p.optString("source")=="driver_inbox")) } }
    }
    suspend fun alertAction(id:String, action:String): Result<Unit> {
        val r=api.bridgePost("/api/driver/mobile/resources", JSONObject().put("action",action).put("notificationId",id))
        if(!r.successful) return Result.failure(IllegalStateException(runCatching{JSONObject(r.body).optString("error")}.getOrDefault("").ifBlank{"Request failed (${r.status})."}))
