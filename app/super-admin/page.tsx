...
function CommandCentre() {
  ...
  const load = useCallback(async () => {
    setLoading(true); setError(null); setStatsUnavailable(false);
    ...
    try {
      ...
      else { setStats(null); setStatsUnavailable(true); }
  ...
  return <div style={{ minHeight: '100vh', background: X.light, color: X.charcoal, padding: '12px' }}>
    ...
    <section style={{ marginBottom: '12px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(170px,1fr))', gap: '12px' }}>
        {(loading ? Array.from({ length: 4 }, (_, i) => [`Loading ${i}`, '—', 'Loading…', '#'] as const) : stats ? kpis : (['Active companies', 'Open jobs', 'Pending approvals', 'Unpaid invoices'] as const).map((label) => [label, '—', statsUnavailable ? 'Summary source unavailable' : 'No summary data', '#'] as const)).map(([label, value, note, href]) => <Link key={label} href={href} style={{ minHeight: '88px', display: 'block', textDecoration: 'none', background: X.white, border: `1px solid ${X.border}`, borderRadius: '4px', padding: '12px' }}><div style={{ color: X.navy, fontSize: '22px', lineHeight: 1.05, fontWeight: 800 }}>{value}</div><div style={{ marginTop: '7px', color: X.charcoal, fontSize: '12px', fontWeight: 700 }}>{label}</div><div style={{ marginTop: '2px', color: X.muted, fontSize: '10px' }}>{note}</div></Link>)}
      </div>
    </section>
    ...
    <section style={{ minHeight: '52px', border: `1px solid ${X.border}`, borderRadius: '4px', background: X.white, padding: '10px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}><div><h2 style={{ margin: 0, color: X.navy, fontSize: '13px', fontWeight: 800 }}>Audit trail</h2><p style={{ margin: '2px 0 0', color: X.muted, fontSize: '11px' }}>Review recorded governance decisions and administrative changes.</p></div><Link href="/super-admin/settings/audit-logs" style={{ color: X.blue, fontSize: '11px', fontWeight: 800, textDecoration: 'none' }}>Open audit trail →</Link></section>
  </div>;
}
