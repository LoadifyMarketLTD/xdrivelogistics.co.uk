...
const actionSchema = z.discriminatedUnion('action', [
  ...
  z.object({ action: z.literal('convert_to_job'), executionMode: z.enum(['own_fleet', 'direct_carrier', 'marketplace']) }),
]);
...
const dateTimeFromNotes = (notes: string | null, dateLabel: string, timeLabel: string) => {
  ...
  return `${date}T${timeForSlot(fieldFromNotes(notes, timeLabel))}:00.000Z`;
};
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const owner = await verifyPlatformOwner(request);
  if (!owner) {
    if (isSuperAdminDeployPreviewReadOnly()) return respond(403, { error: 'Deploy Preview is read-only. XDrive enquiry data was not changed.' });
  ...
  if (action.action === 'set_price') {
    ...
    const { data, error } = await supabaseAdmin.from('quotes')
      ...
      .eq('id', id).eq('company_id', INTAKE_COMPANY_ID)
      .select('id,amount,currency,status,updated_at').single();
    if (error) return respond(500, { error: error.message });
    return respond(200, { enquiry: data });
  }
  ...
  if (action.action === 'quote_sent') {
    ...
    const { data, error } = await supabaseAdmin.from('quotes')
      ...
      .eq('id', id).eq('company_id', INTAKE_COMPANY_ID)
      .select('id,amount,currency,status,quote_sent_at,updated_at').single();
    if (error) return respond(500, { error: error.message });
    return respond(200, { enquiry: data });
  }
  ...
  if (action.action === 'accepted') {
    ...
    const { data, error } = await supabaseAdmin.from('quotes')
      ...
      .eq('id', id).eq('company_id', INTAKE_COMPANY_ID)
      .select('id,amount,currency,status,accepted_at,updated_at').single();
    if (error) return respond(500, { error: error.message });
    return respond(200, { enquiry: data });
  }

  if (enquiry.converted_job_id) return respond(200, { job: { id: enquiry.converted_job_id }, replayed: true });
  if (String(enquiry.status ?? '').toLowerCase() !== 'accepted') return respond(409, { error: 'The customer quote must be accepted before conversion to a job.' });
  ...
  const jobRow: Record<string, unknown> = {
    ...
    load_details: { source: 'xdrive_public_enquiry', enquiryId: enquiry.id, executionMode: action.executionMode, sourceNotes: notes },
  ...
  const existingJob = await supabaseAdmin.from('jobs')
  ...
  const { error: quoteUpdateError } = await supabaseAdmin.from('quotes')
    .update({ status: 'converted', converted_at: now, converted_job_id: job.id, execution_mode: action.executionMode, updated_at: now })
    .eq('id', id).eq('company_id', INTAKE_COMPANY_ID);
  if (quoteUpdateError) {
    return respond(500, {
      error: quoteUpdateError.message,
      job,
      code: 'enquiry_conversion_reconciliation_required',
      note: 'The job exists but the source enquiry could not be marked converted. Manual reconciliation is required before retrying.',
    });
  }
