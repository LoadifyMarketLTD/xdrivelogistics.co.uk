...
export default function MarketplacePage() {
  ...
  const loadMyBids = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('job_bids')
      .select('id, job_id, company_id, amount, bid_price_gbp, currency, message, status, created_at, jobs(id, pickup_location, delivery_location, pickup_datetime, vehicle_type, company_id, companies:company_id(name))')
      .eq('company_id', companyId)
  ...
  const loadWonJobs = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('jobs')
      .select('id, pickup_location, delivery_location, pickup_datetime, vehicle_type, status, currency, budget_amount, company_id, awarded_carrier_company_id, created_at, companies:company_id(name)')
