alter table public.driver_mobile_device_sessions
  add column if not exists auth_session_created_at timestamptz;

update public.driver_mobile_device_sessions d
set auth_session_created_at = coalesce(
  (select s.created_at from auth.sessions s where s.id = d.auth_session_id and s.user_id = d.user_id),
  d.registered_at
)
where d.auth_session_created_at is null;

alter table public.driver_mobile_device_sessions
  alter column auth_session_created_at set not null;

comment on column public.driver_mobile_device_sessions.auth_session_created_at is
  'Creation time of the validated Supabase Auth session. Used to ensure an older session can never replace a newer native login.';
...
begin
  ...
  -- Serialize registration decisions for one driver so two simultaneous logins
  -- cannot both observe themselves as the newest session.
