...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Collection evidence is part of the lifecycle transition itself, so link it
  // immediately. Delivery POD evidence remains unlinked until the final POD POST
  // verifies every uploaded object and atomically records the POD metadata.
  if (kind === 'collection') {
