...
export async function POST(request: NextRequest) {
  ...
  try {
    if (event.type === 'checkout.session.completed' && object.mode === 'subscription') {
      ...
      let lookup = supabaseAdmin.from('platform_membership_subscriptions').select('id, plan_id, trial_ends_at');
      lookup = companyId ? lookup.eq('company_id', companyId) : lookup.eq('user_id', userId).is('company_id', null);
    ...
    if (event.type === 'invoice.payment_failed' || event.type === 'invoice.paid') {
      ...
      if (subscriptionId) {
        ...
        if (event.type === 'invoice.paid') {
          const { data: row, error: rowError } = await supabaseAdmin.from('platform_membership_subscriptions').select('status, trial_ends_at').eq('stripe_subscription_id', subscriptionId).maybeSingle();
          if (rowError) throw rowError;
          if (row?.status === 'trialing' && isFutureTimestamp(row.trial_ends_at)) nextStatus = 'trialing';
        }
        const { error } = await supabaseAdmin.from('platform_membership_subscriptions').update({ status: nextStatus, updated_at: new Date().toISOString() }).eq('stripe_subscription_id', subscriptionId);
        if (error) throw error;
      }
      await finishEvent(event.id, 'processed');
      return json(200, { received: true });
