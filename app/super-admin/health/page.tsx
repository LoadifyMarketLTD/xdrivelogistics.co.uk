  ...
  const runChecks = useCallback(async () => {
    ...
    const checkApi = async ({ service, url }: { service: string; url: string }): Promise<ServiceCheck> => {
      ...
      try {
        ...
        return { service, status: 'error', latencyMs: Date.now() - start, detail: 'Endpoint unavailable.' };
      }
    };
      ...
      <div style={{ minHeight: '100vh', background: X.light, color: X.charcoal, padding: '12px' }}>
        <header style={{ minHeight: '52px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', marginBottom: '12px', flexWrap: 'wrap' }}>
          <div>
            ...
            <p style={{ color: X.muted, margin: '4px 0 0', fontSize: '12px' }}>Live internal service checks and integration readiness.</p>
        ...
        <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: '12px', marginBottom: '16px' }}>
          {loading && checks.length === 0 ? Array.from({ length: 8 }).map((_, i) => <div key={i} style={{ background: X.white, border: `1px solid ${X.border}`, borderRadius: '4px', padding: '12px', color: X.muted, fontSize: '11px' }}>Checking…</div>) : checks.map((check) => <HealthCard key={check.service} check={check} />)}
        </section>
