...
function hasCompactKpiStrip(filePath: string): boolean {
  const source = read(filePath);
  return /\bExchangeKpiStrip\b|\bKpiGrid\b|\bCarrierControlSignals\b|\bOperationalSignalStrip\b|customer-dash-metrics|driver-dashboard-status|driver-proto-kpis/.test(source);
}
...
function rowFor(filePath: string) {
  ...
  return {
    ...
    exchangeKpiStrip: /\bExchangeKpiStrip\b|\bKpiGrid\b|\bCarrierControlSignals\b|\bOperationalSignalStrip\b|customer-dash-metrics|driver-dashboard-status|driver-proto-kpis/.test(source),
