...
export default function Page() {
  ...
  const columns: SuperAdminDataColumn<Company>[] = [
    { key:'company', label:'Company', render:(row)=><div><PlatformEntityLink entityType="company" entityId={row.id} compact>{row.name}</PlatformEntityLink><div style={{fontSize:11,color:'#64748B'}}>{row.email ?? 'No email'}</div></div> },
