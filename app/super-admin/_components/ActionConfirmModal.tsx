...
export function ActionConfirmModal({open,title,description,confirmLabel,cancelLabel='Cancel',danger=true,reasonLabel='Reason (required)',reasonRequired=true,reasonPlaceholder='Describe the reason for this action…',submitting=false,onConfirm,onCancel}:ActionConfirmModalProps){
 ...
 return <div role='dialog' aria-modal='true' aria-labelledby='acm-title' style={{position:'fixed',inset:0,zIndex:1000,backgroundColor:'rgba(74,74,74,.24)',display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}} onClick={e=>{if(e.target===e.currentTarget)onCancel();}}>
  <div style={{backgroundColor:X.white,border:`1px solid ${X.border}`,borderTop:`4px solid ${confirmColor}`,borderRadius:'12px',padding:'20px',width:'100%',maxWidth:'520px',boxShadow:'0 20px 54px rgba(31,41,55,.16)',fontFamily:'Roboto, Inter, Arial, sans-serif',color:X.charcoal}}>
   <h2 id='acm-title' style={{margin:'0 0 8px',fontFamily:'Inter, Roboto, Arial, sans-serif',fontSize:'20px',lineHeight:1.25,fontWeight:800,color:X.blue}}>{title}</h2>
   ...
   <div style={{display:'flex',gap:'10px',marginTop:'18px',justifyContent:'flex-end'}}><button type='button' onClick={onCancel} disabled={submitting} style={{minHeight:'40px',padding:'0 14px',borderRadius:'8px',border:`1px solid ${X.border}`,backgroundColor:X.white,color:X.blue,fontFamily:'Inter, Roboto, Arial, sans-serif',fontSize:'14px',fontWeight:700,cursor:submitting?'not-allowed':'pointer'}}>{cancelLabel}</button><button type='button' onClick={()=>{if(!isDisabled)onConfirm(reason.trim());}} disabled={isDisabled} style={{minHeight:'40px',padding:'0 14px',borderRadius:'8px',border:`1px solid ${confirmColor}`,backgroundColor:confirmColor,color:X.white,fontFamily:'Inter, Roboto, Arial, sans-serif',fontWeight:800,fontSize:'14px',cursor:isDisabled?'not-allowed':'pointer',opacity:isDisabled?.55:1}}>{submitting?'…':confirmLabel}</button></div>
  </div>
 </div>;
}
