    ...
    <ProtectedRoute>
      <div style={{ display: 'flex', height: 'calc(100vh - 89px)', overflow: 'hidden', background: '#f5f7fa' }}>
        <aside style={{ width: '200px', flexShrink: 0, background: '#fff', borderRight: '1px solid #e2e8f0', padding: '0.85rem', overflowY: 'auto', fontSize: '0.78rem' }}>
          <div style={{ fontWeight: 700, color: '#0f172a', marginBottom: '0.6rem', fontSize: '0.8rem' }}>ðŸ“‹ Search Panel</div>
          ...
          <div style={{ marginBottom: '0.55rem' }}><div style={labelStyle}>Load ID / Ref</div><input placeholder="Searchâ€¦" style={panelInput} /></div>
          ...
          <div style={{ marginBottom: '0.85rem' }}><div style={labelStyle}>Customer Name</div><input placeholder="Searchâ€¦" style={panelInput} /></div>
        ...
        <main style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
          <div style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '0.45rem 1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, gap: '1rem' }}><div style={{ fontSize: '0.82rem', color: '#374151', fontWeight: 600 }}>ðŸ“… Diary â€” {jobs.length} bookings</div><button onClick={() => void loadJobs()} style={smallGhostButton}>â†» Refresh</button></div>
          ...
          <div style={{ padding: '0.75rem', flex: 1, overflowY: 'auto' }}>
            {loading ? (<div style={emptyCard}>Loading diaryâ€¦</div>) : filteredJobs.length === 0 ? (<div style={emptyCard}><div style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>ðŸ“‹</div><div style={{ fontSize: '0.88rem' }}>No bookings in this category.</div></div>) : (
                  ...
                  return (
                    <div key={job.id} style={{ background: '#fff', border: '1px solid #e2e8f0', borderLeft: `3px solid ${badge.color === '#15803d' || badge.color === '#166534' ? '#16a34a' : badge.color === '#991b1b' ? '#ef4444' : '#64748b'}`, borderRadius: '6px', overflow: 'hidden' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '0.75rem', padding: '0.75rem 1rem', alignItems: 'start' }}>
                        <div><div style={{ display: 'flex', gap: '0.3rem', alignItems: 'baseline' }}><span style={{ fontSize: '0.65rem', color: '#94a3b8', fontWeight: 700, minWidth: '28px' }}>From:</span><span style={{ fontWeight: 700, color: '#0f172a', fontSize: '0.85rem' }}>{job.client_name || job.booked_by_company_name || 'Contact N/A'}</span></div><div style={{ fontSize: '0.8rem', color: '#374151', marginLeft: '36px', marginTop: '0.1rem' }}>{job.pickup_location || 'â€”'}</div><div style={{ display: 'flex', gap: '0.3rem', alignItems: 'baseline', marginTop: '0.4rem' }}><span style={{ fontSize: '0.65rem', color: '#94a3b8', fontWeight: 700, minWidth: '28px' }}>To:</span><span style={{ fontWeight: 600, color: '#374151', fontSize: '0.82rem' }}>{job.delivery_location || 'â€”'}</span></div></div>
                        <div><div style={{ display: 'flex', gap: '0.3rem', alignItems: 'baseline' }}><span style={{ fontSize: '0.65rem', color: '#94a3b8', fontWeight: 700, minWidth: '58px' }}>Pickup:</span><span style={{ fontSize: '0.8rem', color: '#374151' }}>{formatDate(job.pickup_datetime ?? job.updated_at)}</span></div><div style={{ display: 'flex', gap: '0.3rem', alignItems: 'baseline', marginTop: '0.2rem' }}><span style={{ fontSize: '0.65rem', color: '#94a3b8', fontWeight: 700, minWidth: '58px' }}>Vehicle:</span><span style={{ fontSize: '0.78rem', color: '#64748b' }}>{job.vehicle_type ? job.vehicle_type.replace(/_/g, ' ') : 'â€”'}</span></div><div style={{ marginTop: '0.35rem', fontSize: '0.68rem', color: '#64748b', lineHeight: 1.35 }}>{job.on_my_way_at && <div>On way: {formatDateTime(job.on_my_way_at)}</div>}{job.on_site_pickup_at && <div>Pickup site: {formatDateTime(job.on_site_pickup_at)}</div>}{job.loaded_at && <div>Loaded: {formatDateTime(job.loaded_at)}</div>}{job.on_site_delivery_at && <div>Delivery site: {formatDateTime(job.on_site_delivery_at)}</div>}{job.delivered_at && <div>Delivered: {formatDateTime(job.delivered_at)}</div>}</div></div>
      ...
      {activeModal && selectedJob && (<div style={modalBackdrop}><div style={modalBox}><div style={modalHeader}><strong>{activeModal === 'order' && `Order â€” ${(selectedJob.load_id || selectedJob.id).slice(0, 8).toUpperCase()}`}{activeModal === 'notes' && 'Internal Notes'}{activeModal === 'history' && `History â€” ${(selectedJob.load_id || selectedJob.id).slice(0, 8).toUpperCase()}`}{activeModal === 'documents' && 'Load Documents'}{activeModal === 'pod' && 'Upload POD'}</strong><button onClick={closeModal} style={closeButton}>Ã—</button></div><div style={modalBody}>
        ...
        {activeModal === 'history' && (<div style={{ display: 'grid', gap: '0.55rem' }}><TimelineRow label="Current status" value={STATUS_BADGE[normalizeStatus(selectedJob.status)]?.label || selectedJob.status} /><TimelineRow label="On My Way To Pickup" value={formatDateTime(selectedJob.on_my_way_at)} /><TimelineRow label="On Site Pickup" value={formatDateTime(selectedJob.on_site_pickup_at)} /><TimelineRow label="Loaded" value={formatDateTime(selectedJob.loaded_at)} /><TimelineRow label="On Site Delivery" value={formatDateTime(selectedJob.on_site_delivery_at)} /><TimelineRow label="Delivered" value={formatDateTime(selectedJob.delivered_at)} /><TimelineRow label="Completed" value={formatDateTime(selectedJob.completed_at)} />
                  <TimelineRow label="Operational notes" value={selectedJob.delivery_notes || selectedJob.collection_notes || selectedJob.load_notes || 'â€”'} /><div style={{ marginTop: '0.6rem' }}><div style={smallTitle}>Raw status history</div><pre style={preBox}>{JSON.stringify(selectedJob.status_history ?? [], null, 2)}</pre></div></div>)}
        {activeModal === 'documents' && (<><div style={formRow}><input value={documentName} onChange={(event) => setDocumentName(event.target.value)} placeholder="Document name / type" style={textInput} /><input type="file" onChange={(event) => setDocumentFile(event.target.files?.[0] ?? null)} style={textInput} /><button onClick={() => void handleUploadDocument()} disabled={modalBusy || !documentFile} style={greenButton}>{modalBusy ? 'Uploadingâ€¦' : 'Upload'}</button></div><div style={{ marginTop: '1rem', display: 'grid', gap: '0.55rem' }}>{documents.length === 0 ? <div style={mutedText}>No documents uploaded yet.</div> : documents.map((doc) => (<div key={doc.id} style={documentRow}><div><div style={{ fontWeight: 700 }}>{doc.file_type || fileNameFromPath(doc.file_url)}</div><div style={{ fontSize: '0.75rem', color: '#64748b' }}>{formatDateTime(doc.created_at || doc.uploaded_at)}</div></div><div style={{ display: 'flex', gap: '0.4rem' }}><button onClick={() => void handleDownloadDocument(doc)} style={smallGhostButton}>Download</button><button onClick={() => void handleDeleteDocument(doc)} style={dangerButton}>Delete</button></div></div>))}</div></>)}
        {activeModal === 'pod' && (<><div style={grid2}><div><div style={labelStyle}>Delivery photo / POD file</div><input type="file" accept="image/*,.pdf" onChange={(event) => setPodFile(event.target.files?.[0] ?? null)} style={textInput} /></div><div><div style={labelStyle}>Recipient name</div><input value={podRecipientName} onChange={(event) => setPodRecipientName(event.target.value)} placeholder="Name of recipient" style={textInput} /></div><div><div style={labelStyle}>Signature / signed by</div><input value={podSignature} onChange={(event) => setPodSignature(event.target.value)} placeholder="Typed signature" style={textInput} /></div><div><div style={labelStyle}>Existing POD</div><div style={mutedText}>Photos: {safeArray(selectedJob.delivery_photos).length + safeArray(selectedJob.pod_photos).length}</div></div><div style={{ gridColumn: '1 / -1' }}><div style={labelStyle}>Delivery notes</div><textarea value={podNotes} onChange={(event) => setPodNotes(event.target.value)} placeholder="Delivery notesâ€¦" style={textareaStyle} /></div></div><div style={{ marginTop: '0.75rem', display: 'flex', gap: '0.5rem' }}><button onClick={() => void handleSavePod()} disabled={modalBusy} style={greenButton}>{modalBusy ? 'Savingâ€¦' : 'Save POD'}</button></div></>)}
      </div></div></div>)}
    </ProtectedRoute>
  );
}

function Info({ label, value }: { label: string; value: string | number | null | undefined }) { return (<div style={infoBox}><div style={labelStyle}>{label}</div><div style={{ color: '#0f172a', fontWeight: 650, whiteSpace: 'pre-wrap' }}>{value || 'â€”'}</div></div>); }
function TimelineRow({ label, value }: { label: string; value: string }) { return (<div style={listItem}><div style={{ fontWeight: 800, color: '#0f172a' }}>{label}</div><div style={{ color: value === 'â€”' ? '#94a3b8' : '#334155', marginTop: '0.15rem' }}>{value}</div></div>); }
