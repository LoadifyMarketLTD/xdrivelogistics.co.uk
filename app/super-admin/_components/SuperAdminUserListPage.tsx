...
export default function SuperAdminUserListPage({icon,title,description,section,roleFilter,columns}:SuperAdminUserListPageProps){
 ...
 return <ProtectedRoute allowedRoles={['owner']}><div style={{minHeight:'100vh',background:X.light,color:X.charcoal,padding:12}}>
  ...
  {!error&&<>
   ...
   <section style={{background:X.white,border:`1px solid ${X.border}`,borderRadius:4,overflow:'hidden'}}><div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}><thead><tr style={{height:38,background:X.light,borderBottom:`1px solid ${X.border}`}}>{columns.map(col=><th key={col.label} style={{padding:'0 12px',textAlign:'left',color:X.navy,fontWeight:800,textTransform:'uppercase',letterSpacing:'.04em',fontSize:10,whiteSpace:'nowrap'}}>{col.label}</th>)}<th style={{padding:'0 12px',textAlign:'left',color:X.navy,fontWeight:800,textTransform:'uppercase',letterSpacing:'.04em',fontSize:10,whiteSpace:'nowrap'}}>Inspect</th></tr></thead><tbody>{loading?<tr><td colSpan={columnCount} style={{padding:18,textAlign:'center',color:X.muted}}>Loading {title.toLowerCase()}…</td></tr>:filtered.length===0?<tr><td colSpan={columnCount} style={{padding:18,textAlign:'center',color:X.muted}}>{filter?'No records on this page match the filter.':`No ${title.toLowerCase()} found.`}</td></tr>:filtered.map(row=><tr key={row.id} style={{minHeight:44,borderBottom:`1px solid ${X.border}`}}>{columns.map(col=><td key={col.label} style={{padding:'9px 12px',color:X.charcoal,verticalAlign:'middle'}}>{col.render(row)}</td>)}<td style={{padding:'9px 12px',verticalAlign:'middle'}}>{row.user_id?<PlatformEntityLink entityType='user' entityId={row.user_id} compact>Open</PlatformEntityLink>:<span style={{color:X.muted,fontSize:10}}>Invite only</span>}</td></tr>)}</tbody></table></div>
