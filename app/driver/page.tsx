...
export default function DriverDashboard() {
  ...
  const myJobs = useMemo(() => data.jobs.filter((job) => Boolean(user?.driverId) && job.assigned_driver_id === user?.driverId), [data.jobs, user?.driverId]);
  ...
  const todaysJobs = myJobs.filter((job) => job.pickup_datetime && new Date(job.pickup_datetime).toDateString() === new Date().toDateString());
  const myDocuments = data.driverDocuments.filter((document) => Boolean(user?.driverId) && document.driver_id === user?.driverId);
  const expiringDocuments = myDocuments.filter((document) => document.expiry_date && new Date(document.expiry_date).getTime() < Date.now() + 30 * 86_400_000).length;
  const completedJobs = myJobs.filter((job) => ['delivered', 'completed', 'invoiced', 'paid'].includes(job.status)).length;

  return (
    <PageFrame>
      <PageHeader eyebrow="Driver operations" title="Driver Dashboard" description="See today’s assigned work, the next required action, availability and document readiness." actions={<><ActionButton tone="secondary" onClick={() => router.push('/driver/availability')}>Availability</ActionButton><ActionButton tone="secondary" onClick={() => router.push('/driver/documents')}>Documents</ActionButton></>} />
      ...
      <TwoColumn>
        <Panel title={currentJob ? 'Current job' : 'Next operational work'} description="The job card shows the authoritative route, timing and next driver action.">
          {currentJob ? <div style={{ display: 'grid', gap: '0.8rem' }}><div><strong style={{ display: 'block', fontSize: '1rem', color: '#0f172a' }}>{currentJob.pickup_location ?? 'Collection'} → {currentJob.delivery_location ?? 'Delivery'}</strong><span style={{ display: 'block', color: '#64748b', fontSize: '0.78rem', marginTop: '0.3rem' }}>Pickup {formatDateTime(currentJob.pickup_datetime)} · Delivery {formatDateTime(currentJob.delivery_datetime)}</span></div><StatusBadge value={currentJob.current_status ?? currentJob.status} /><ActionButton tone="success" onClick={() => router.push(`/driver/jobs/${currentJob.id}`)}>Open job and actions</ActionButton></div> : <EmptyState title="No active job" description="Assigned work appears here as soon as it is allocated." />}
        </Panel>
