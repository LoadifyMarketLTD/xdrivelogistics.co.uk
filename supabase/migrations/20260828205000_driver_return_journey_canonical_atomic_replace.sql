-- Canonical atomic Return Journey replacement for the Expo driver application.
--
-- The PR #357 runtime schema uses from_postcode / to_postcode / available_from /
-- available_to / vehicle_type / notes / status. The earlier 27 Aug helper still
-- targeted historical from_location / to_location / available_date columns.
-- This new server-only RPC operates only on the canonical schema and makes clear
-- and replacement transactional so a failed insert can never destroy the
-- driver's previously published journey.
...
BEGIN
  ...
  -- Defense in depth: service-role callers still cannot bind a driver journey to
  -- an unrelated company merely by passing arbitrary UUIDs.
  IF NOT EXISTS (
    SELECT 1
  ...
  -- rejected for any reason, PostgreSQL rolls the delete back automatically and
  -- the previously published journey remains intact.
  DELETE FROM public.return_journeys
  WHERE driver_id = p_driver_id;
