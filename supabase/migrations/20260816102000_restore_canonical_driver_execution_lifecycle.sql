-- Restore the approved canonical driver execution lifecycle after the later
-- native-status RPC definition regressed loaded -> in_transit -> on_site_delivery
-- into loaded -> on_site_delivery. This is a contract repair only: no new roles,
-- grants, tables, policies or lifecycle states are introduced.
