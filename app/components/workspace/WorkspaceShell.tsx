...
export default function WorkspaceShell({ children, forcedRole }: { children: ReactNode; forcedRole?: WorkspaceRole }) {
  ...
  const settingsRoute = isEnterpriseSettingsRoute(pathname, role);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    supabase.from('companies').select('name').eq('id', user.companyId).maybeSingle().then(({ data }) => {
  ...
  useEffect(() => {
    ...
    const fetchUnread = async () => {
      const { count } = await supabase.from('notification_events').select('id', { count: 'exact', head: true }).eq('recipient_user_id', user.id).in('status', ['pending', 'failed']);
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: workspaceTheme.page, color: workspaceTheme.text }}>
      ...
      <aside style={sidebarStyle} aria-label={`${definition.label} navigation`}>
        <div style={{ padding: '0.75rem', background: '#fff', borderBottom: `1px solid ${workspaceTheme.border}` }}>
          <button onClick={() => router.push(definition.homeHref)} style={{ border: 0, background: 'transparent', padding: 0, width: '100%', cursor: 'pointer', textAlign: 'left' }}>
            <div style={{ display: 'flex', gap: '0.55rem', alignItems: 'center' }}>
              <div style={{ width: '34px', height: '34px', borderRadius: '8px', background: workspaceTheme.navy, display: 'grid', placeItems: 'center', flexShrink: 0, boxShadow: '0 2px 6px rgba(11,47,107,0.18)' }}><span style={{ color: workspaceTheme.orange, fontWeight: 950, fontSize: '1rem' }}>X</span></div>
              <div style={{ minWidth: 0 }}><div style={{ color: workspaceTheme.text, fontSize: '0.8rem', fontWeight: 850, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{companyName}</div><div style={{ color: workspaceTheme.muted, fontSize: '0.62rem', marginTop: '0.08rem' }}>{definition.subtitle}</div></div>
            </div>
          </button>
        ...
        <nav style={{ flex: 1, overflowY: 'auto', padding: '0.48rem' }}>
          {nav.map((group) => <div key={group.id} style={{ marginBottom: '0.42rem' }}><div style={{ padding: '0.25rem 0.42rem 0.18rem', color: '#64748b', fontSize: '0.58rem', fontWeight: 850, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{group.label}</div><div style={{ display: 'grid', gap: '0.1rem' }}>{group.items.map((item) => { const active = isActive(item.href); return <button key={item.id} onClick={() => { router.push(item.href); if (isCompact) setSidebarOpen(false); }} style={{ width: '100%', display: 'grid', gridTemplateColumns: '22px minmax(0,1fr) 7px', alignItems: 'center', gap: '0.34rem', border: 0, borderLeft: active ? `3px solid ${workspaceTheme.blue}` : '3px solid transparent', borderRadius: '7px', background: active ? '#eff6ff' : 'transparent', color: active ? workspaceTheme.blue : workspaceTheme.text, padding: '0.4rem 0.45rem', fontSize: '0.7rem', fontWeight: active ? 850 : 650, textAlign: 'left', cursor: 'pointer' }}><span aria-hidden="true" style={{ width: '21px', height: '21px', borderRadius: '6px', display: 'grid', placeItems: 'center', background: active ? '#dbeafe' : '#eef2f6', color: active ? workspaceTheme.blue : '#475569', fontSize: item.icon === 'OC' ? '0.52rem' : '0.68rem', fontWeight: 900 }}>{item.icon ?? '•'}</span><span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.label}</span>{active ? <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: workspaceTheme.blue }} /> : <span />}</button>; })}</div></div>)}
        </nav>
        ...
        <div style={{ padding: '0.65rem', borderTop: `1px solid ${workspaceTheme.border}`, background: '#fff' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.34rem' }}><button onClick={() => router.push(definition.homeHref)} style={{ border: `1px solid ${workspaceTheme.border}`, borderRadius: '7px', background: '#fff', color: workspaceTheme.text, padding: '0.4rem', fontSize: '0.65rem', fontWeight: 800, cursor: 'pointer' }}>Home</button><button onClick={() => void logout()} style={{ border: '1px solid #fecaca', borderRadius: '7px', background: '#fff', color: workspaceTheme.red, padding: '0.4rem', fontSize: '0.65rem', fontWeight: 800, cursor: 'pointer' }}>Sign out</button></div>
        </div>
      </aside>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        <header style={{ minHeight: '56px', background: '#fff', borderBottom: `1px solid ${workspaceTheme.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.56rem clamp(0.75rem,2vw,1.2rem)', position: 'sticky', top: 0, zIndex: 35, gap: '0.75rem', boxShadow: '0 1px 5px rgba(15,23,42,0.04)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.58rem', minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}><div style={{ fontSize: '0.65rem', color: workspaceTheme.muted, fontWeight: 750 }}>{settingsRoute ? 'Enterprise Settings' : definition.label}</div><div style={{ fontSize: '0.78rem', color: workspaceTheme.text, fontWeight: 850, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{companyName}</div></div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.42rem', flexShrink: 0 }}>
            {!settingsRoute && primaryAction && <button onClick={() => router.push(primaryAction.href)} style={{ border: 0, background: workspaceTheme.orange, color: '#172033', padding: '0.48rem 0.72rem', borderRadius: '8px', fontSize: '0.69rem', fontWeight: 850, cursor: 'pointer', boxShadow: '0 2px 7px rgba(245,163,0,0.22)' }}>+ {primaryAction.label}</button>}
