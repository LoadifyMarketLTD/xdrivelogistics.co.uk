...
async function persistUri(userId: string, jobId: string, uri: string, index: number, fallbackExtension: string) {
  ...
  // Server storage paths do not contain a URI scheme and are already durable.
...
/**
 * Copies queued POD evidence out of Expo/ImagePicker/DocumentPicker cache and
 * into the app document directory before the AsyncStorage queue is committed.
 * documentDirectory survives process restarts and cache eviction until the app
 * explicitly deletes these files.
 */
