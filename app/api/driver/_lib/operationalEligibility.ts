...
export async function resolveDriverOperationalEligibility(
  ...
  const [identityResult, onboardingResult, companyResult, membershipResult, vehiclesResult] = await Promise.all([
    userId
      ? supabaseAdmin
          .from('platform_identity_registry')
          .select('user_id,company_id,identity_mode,status,verified_at')
          .eq('user_id', userId)
          .maybeSingle()
    ...
    userId && companyId
      ? supabaseAdmin
          ...
          .select('company_id,user_id,status,role_in_company')
          .eq('user_id', userId)
          .eq('company_id', companyId)
          .maybeSingle()
      : Promise.resolve({ data: null, error: null }),
  ...
  const firstError = identityResult.error
