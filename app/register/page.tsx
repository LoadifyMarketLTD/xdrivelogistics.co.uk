...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      setMessage('Account created. Open the confirmation email, then continue with the onboarding for the account type you selected. If the email is already registered, use Sign in or Forgot password.');
      setEmail('');
      setPassword('');
      setConfirmPassword('');
    } catch (reason) {
  ...
  return (
    <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: 'linear-gradient(135deg, #0b2f6b 0%, #1d57d8 100%)', padding: '1rem' }}>
      <section style={{ width: '100%', maxWidth: 560, padding: '2rem', background: '#fff', borderRadius: 14, boxShadow: '0 18px 45px rgba(0,0,0,0.2)' }}>
        ...
        <p style={{ color: '#64748b', lineHeight: 1.5, marginTop: 0 }}>Choose the account type that describes what you actually do on the platform. This choice controls your onboarding and workspace.</p>
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div style={{ display: 'grid', gap: '0.65rem', marginTop: '0.75rem' }}>
              {ACCOUNT_TYPE_OPTIONS.map((option) => (
                <button
                  ...
                  style={{ textAlign: 'left', border: accountType === option.value ? '2px solid #1d57d8' : '1px solid #d1d5db', borderRadius: 8, background: '#fff', padding: '0.75rem', cursor: 'pointer' }}
                >
          ...
          {warning && <p role="alert" style={{ color: '#92400e', background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 8, padding: '0.75rem' }}>{warning}</p>}
