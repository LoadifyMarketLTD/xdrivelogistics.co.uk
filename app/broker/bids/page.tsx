...
export default function BrokerQuotesPage() {
  ...
  const rows = useMemo(() => {
    ...
    return data.bids
      ...
      .map((bid) => ({ bid, job: jobs.get(bid.job_id) }))
      ...
      .filter(({ bid }) => !carrierTerm || String(bid.companies?.name || '').toLowerCase().includes(carrierTerm))
      .filter(({ bid, job }) => !referenceTerm || `${bid.id} ${job?.id || ''}`.toLowerCase().includes(referenceTerm))
      .sort((a, b) => Number(priceOf(a.bid)) - Number(priceOf(b.bid)));
  }, [carrier, customer, data.bids, data.jobs, from, reference, tab, to]);
  ...
  const award = async (bidId: string) => {
    ...
    setWorking(null);
    if (!response.ok) {
      setMessage(payload.error || 'Unable to award this carrier quote.');
      return;
    }
  ...
  return (
    <PageFrame>
      <PageHeader
        ...
        title="Quotes"
        description="Compare carrier responses against customer load value and award without leaving the broker board."
        actions={<ActionButton tone="secondary" onClick={() => router.push('/broker/compare-quotes')}>Compare view</ActionButton>}
      />
        ...
        <main style={{ minWidth: 0 }}>
          ...
          {rows.length === 0 ? (
            <div className="workspace-panel" style={{ border: '1px solid var(--ws-border)', background: '#fff' }}>
              <EmptyState compact title="No matching quotes" description="Carrier responses will appear here when they match the selected status and filters." />
            </div>
          ) : (
                ...
                return (
                  <article className="workspace-operational-row" key={bid.id} data-state={bid.status}>
                    <div className="workspace-operational-row__top">
                      ...
                      <div className="workspace-operational-cell">
                        ...
                        <div style={{ display: 'flex', gap: 4, alignItems: 'center', flexWrap: 'wrap', marginTop: 3 }}>
                          ...
                          <ActionButton tone="secondary" onClick={() => setExpanded(open ? null : bid.id)}>{open ? 'Close' : 'Open'}</ActionButton>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="workspace-record-meta">
                      ...
                      <span>Quote #{bid.id.slice(0, 8).toUpperCase()}</span>
                      <span>Customer budget: {budget > 0 ? money(budget) : 'Not set'}</span>
                      <span>{margin !== null ? `Est. spread: ${money(margin)}` : 'Spread unavailable'}</span>
                    </div>
                      ...
                      <div className="workspace-record-details">
                        ...
                        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 5 }}>
                          {bid.status === 'submitted' && (
                            <ActionButton tone="success" disabled={working === bid.id} onClick={() => void award(bid.id)}>
                              {working === bid.id ? 'Awarding…' : 'Award carrier'}
                            </ActionButton>
                          )}
